classdef Controller < most.DClass & most.HasClassDataFile & dynamicprops
    ...
    methods
        function obj = Controller(hModel,guiNames,guiNamesInvisible,hWb)
            if nargin < 2
                hWb = [];
            end
            ...
            for i=1:N
                ...
                if ~isempty(hWb)
                    %waitbar(.3 + i*.65/N,hWb); % waitbar internally calls
                    % drawnow. this is called after the creation of every
                    % UI, which slows down the scanimage startup
                    % substantially
                end
            end
    ...
    methods
        ...
        function showAllGUIs(obj)
            ...
            classGuiNames = fieldnames(obj.hGuiClasses);
            for i = length(classGuiNames) % load managed classes
                obj.hGuiClasses.(classGuiNames{i}).showGui();
            end
        ...
        function addGuiClass(obj, name, GuiClass)
            validateattributes(name, {'char'}, {'scalartext'});
            assert(isvarname(name), 'name must be a valid MATLAB variable name');
            assert(~any(strcmp(fieldnames(obj.hGuiClasses), name)),...
                'Duplicate field name for hGuiClasses found.');
            validateattributes(GuiClass, {'most.Gui'}, {'scalar'});
            obj.hGuiClasses.(name) = GuiClass;
        end
    end
    ...
    methods (Hidden)
        ...
        function attachPropBindingsToToolTipStrings(obj)
            hSIBasename = getSIBaseName();
            prefix = ['Command line: ' hSIBasename '.'];
            ...
            %%% Nested functions
            function hSIBasename = getSIBaseName()
                % search for object variable in base workspace
                baseWorkspaceVars = evalin('base','whos');
                
                % match classes
                baseWorkspaceVarClasses = {baseWorkspaceVars.class};
                mask = strcmp(baseWorkspaceVarClasses,class(obj.hModel));
                baseWorkspaceVars = baseWorkspaceVars(mask);
                
                % match handle
                mask = false(size(baseWorkspaceVars));
                for varIdx = 1:numel(baseWorkspaceVars)
                    val = evalin('base',baseWorkspaceVars(varIdx).name);
                    mask(varIdx) = isequal(val,obj.hModel);
                end
                baseWorkspaceVars = baseWorkspaceVars(mask);
                
                if isempty(baseWorkspaceVars)
                    hSIBasename = 'obj';
                else
                    hSIBasename = baseWorkspaceVars(1).name;
                end
            end
        end
    end
    ...
    methods
        ...
        function h = figure(obj,tag,varargin)
            ...
            if tf
                ...
                h = most.idioms.figure('Tag',tag,varargin{:});
                obj.registerGUI(h);
            end
        end
    ...
    methods (Access=private)
        ...
        function layoutGUI(obj,figH)
            ...
            if isfield(obj.guiLayoutInfo,tag)
                ...
                if isfield(layoutInfo,'Units')
                    set(figH,'Units'  ,layoutInfo.Units);
                end
                ...
                if matlab.lang.OnOffSwitchState.on == layoutInfo.Visible
                    % if the figure is tied to a gui class, load the model
                    % first.
                    programmicGui = fieldnames(obj.hGuiClasses);
                    for i = 1:length(programmicGui)
                        guiField = programmicGui{i};
                        guiClass = obj.hGuiClasses.(guiField);
                        if figH == guiClass.hFig
                            guiClass.Visible = true;
                            return;
                        end
                    end
                end
