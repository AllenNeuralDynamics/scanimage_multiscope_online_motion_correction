classdef HasMachineDataFile < most.HasClassDataFile
    ...
    methods
        function deleteAndRemoveMdfHeading(obj)
            heading = obj.custMdfHeading;
    ...
    methods (Hidden)
        % returns the default mdf entries
        % overload if needed
        function s = getMdfDefault(obj)
            s = obj.mdfDefault;
        end
        
        % determines if the mdf is opened in the editor when object is
        % first instantiated
        % overload if needed
        function tf = getOpenMdfInEditor(obj)
            tf = true;
        end
    end
    ...
    methods (Static)
        ...
        % utility function which generates a valid Mdf entry struct
        % makeMdfEntry(comment)
        % makeMdfEntry(name, value)
        % makeMdfEntry(name, value, comment)
        % makeMdfEntry(name, value, comment, liveUpdate)
    ...
    methods (Hidden)
        ...
        function vars = getMDFVars(obj, mdfPath, mdfHeading, customMdfEditor)
            ...
            obj.mdfHeadingCreated = obj.ensureClassAndDependentsAreInMDF(mdfHeading); % instead of class(obj), can also use obj.mdfClassName
            if obj.mdfHeadingCreated
                if isa(obj,'dabs.resources.configuration.HasConfigPage')
                    %No-op
                elseif most.idioms.isValidObj(mdf.hConfigEditor)
                    if ~mdf.hConfigEditor.doModalSectionEdit(mdfHeading)
                        error('MachineDateFile: Operation canceled.');
                    end
                elseif obj.getOpenMdfInEditor()
                    edit(mdf.fileName);
        ...
        function success = safeSetPropFromMdf(obj,propertyName,mdfVariableName,scalingFcn)
            if nargin < 3 || isempty(mdfVariableName)
                mdfVariableName = propertyName;
            end
            
            if nargin < 4 || isempty(scalingFcn)
                scalingFcn = @(v)v;
            end
            ...
            try
                assert(isfield(obj.mdfData,mdfVariableName),'MDF does not contain a variable named ''%s''',mdfVariableName);
        ...
        function tfNewVar = safeWriteVarToHeading(obj,varargin)
            tfNewVar = false;
            try
                tfNewVar = obj.writeVarToHeading(varargin{:});
            catch ME
                most.ErrorHandler.logAndReportError(ME);
            end
        end
        
        function tfNewVar = updateMdfVar(obj,varName,val)
            if nargin < 3
                val = obj.mdfData.(varName);
            end
            
            mdf = most.MachineDataFile.getInstance();
            if mdf.isLoaded
                tfNewVar = mdf.writeVarToHeading(obj.custMdfHeading,varName,val);
                obj.mdfData.(varName) = val;
            end
        end
        ...
        function tf = ensureClassAndDependentsAreInMDF(obj, mdfHeading)
            ...
            for c = 1:numel(allClasses)
                ...
                if ~mdf.isHeading(hding)
                    mdf.generateDefaultSection(obj,hding);
                    tf = true;
                else
                    [~,s] = mdf.getVarsUnderHeading(hding);
                    tf = isempty(s) || isempty(fieldnames(s));
                    if tf
                         mdf.generateDefaultSection(obj,hding);
                    end
                end
    ...
    methods (Static,Hidden)        
        ...
        function status = ensureMDFExists(mdfPath)
            ...
            if mdf.isLoaded && (isempty(mdfPath) || strcmp(mdfPath,mdf.fMDFName))
                status = 'exist';
                return;
            end
        ...
        function deleteClassDataFile()
            most.HasClassDataFile.deleteClassDataFile('most.HasMachineDataFile');
        end
    end  
    
    %%% Property Getter/Setter
    methods
        function val = get.custMdfHeading(obj)
            if isa(obj,'dabs.resources.Resource')
                val = sprintf('%s (%s)', class(obj), obj.name);
            else
                val = obj.custMdfHeading;
            end
        end
    end
end
