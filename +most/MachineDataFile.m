classdef MachineDataFile < handle
    ...
    methods
        ...
        function load(obj,fname)
            ...
            obj.fMDFName = fname;
            obj.fHData = hd;
            obj.suppressUnloadWarning = false;
        end
        ...
        function tf = assertLoaded(obj)
            tf = obj.isLoaded;
            
            if ~tf && ~obj.suppressUnloadWarning
                obj.suppressUnloadWarning = true;
                error('Attempted to access MDF after it was unloaded. Future errors will be suppressed.');
            end
        end
        ...
        function tfNewVar = writeVarToHeading(obj,hding,varName,val,custcmt,commitNow)
            ...
            if isa(val,'dabs.resources.InvalidResource')
                % do not attempt to save an invalid resource
                % this potentially overwrites an existing valid setting
                return
            end
        ...
        function tfFound = removeStructFromHeading(obj,hding,sName)
            assert(obj.isLoaded);
            assert(obj.isHeading(hding));
            ...
            for c = 1:numel(rows)
                ...
                vName = strtrim(toks{1}{1});
                if strncmp(sName,vName,l) && ((length(vName)==l) || (vName(l+1) == '(') || (vName(l+1) == '.'))
                    tfFound = true;
                    delRows = [delRows c];
                end
            end
            ...
            if tfFound
                obj.fHData(hdingIdx).rows(delRows) = [];
                most.MachineDataFile.write(obj.fMDFName,obj.fHData);
            end
        end
        ...
        % automatically.
        function generateDefaultSection(obj,hasMDFObj,customHeading)
            assert(obj.isLoaded);
            
            if nargin > 2 && ~isempty(customHeading)
                hdg = customHeading;
            else
                hdg = hasMDFObj.mdfHeading;
            end
            
            s = hasMDFObj.getMdfDefault();
    ...
    methods (Static)
        ...
        function clear
            most.MachineDataFile.getInstance().unload();
        end
