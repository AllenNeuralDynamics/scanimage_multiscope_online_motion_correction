classdef RoiManager < scanimage.interfaces.Component
    ...
    methods (Access = ?scanimage.SI)
        function obj = RoiManager()
            obj@scanimage.interfaces.Component('SI RoiManager');
    ...
    methods
        ...
        function val = get.roiGroupDefault(obj)            
            ...
            scl = [scannerset.transformParams.scaleX scannerset.transformParams.scaleY];
            ...
            scannerset.satisfyConstraintsRoiGroup(obj.roiGroupDefault_);
    ...
    methods(Hidden)
        function updateTimingInformation(obj,setLinePeriod)
            ...
            if ~most.idioms.isValidObj(obj.hSI.hScan2D)
                return
            end
        ...
        function updateLiveNonMroiImaging(obj)
            if ~obj.mroiEnable
                if ~obj.preventLiveUpdate
                    obj.hSI.hScan2D.updateLiveValues([],'RG');
        ...
        function cacheScanTimesPerPlane(obj,varargin)
            ...
            if obj.isLineScan
                ...
                uniqueZs = unique(zs);
                uniqueSliceTimes = arrayfun(@(z)rg.sliceTime(ss,z),uniqueZs);
                
                for idx = N : -1 : 1
                    z = zs(idx);
                    zmask = z==uniqueZs;
                    obj.cachedScanTimesPerPlane(idx) = uniqueSliceTimes(zmask);
                end
            end
        end
        ...
        function coerceDefaultRoi(obj)
            ...
            if isempty(inp)
                try
                    ...
                    sas = (rG.rois.scanfields.centerXY - fovCentroid) ./ [ss.transformParams.scaleX ss.transformParams.scaleY];
