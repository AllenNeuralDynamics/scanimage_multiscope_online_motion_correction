classdef GpuMotionEstimator < scanimage.interfaces.IMotionEstimator
    ...
    methods
        function obj=GpuMotionEstimator(referenceRoiData)
            ...
            emptyMask = cellfun(@(s)isempty(s),scanfields);
            scanfields(emptyMask) = [];
    ...
    methods (Access = protected)
        ...
        function motion_estimator_result = estimateMotionInternal(obj,roiData)
            ...
            zIsRestricted = ~isempty(obj.restrictZs) && ~ismember(roiData.zs,obj.restrictZs);

            if zIsRestricted || isempty(chIdx) || isempty(zIdx) % reference channel or reference z is currently not imaged
    ...
    methods
        ...
        function plotCorrelations(obj,cxxs,cyys)
            ...
            if isempty(hIm_x) || isempty(hIm_y) || ~isvalid(hIm_x) || ~isvalid(hIm_y)
                ...
                hAx_x = most.idioms.subplot(2,1,1,'Parent',hFig);
                hAx_y = most.idioms.subplot(2,1,2,'Parent',hFig);
                view(hAx_x,90,90);
                view(hAx_y,90,90);
