    ...
    methods (Hidden)
        ...
        function reinit(obj)
            ...
            try
                ...
                if isempty(obj.hScan.hBeams)
                    obj.separateBeamDAQ = false;
                    assert(obj.hasGalvos, 'Operation without galvos OR beams is not supported.');
                else
                    beamsError = any(cellfun(@(hR)~isempty(hR.errorMsg),obj.hBeams));
                    assert(~beamsError,'One or more beam devices is in an error mode.');
                    
                    beamDAQs = cellfun(@(hR)hR.hAOControl.hDAQ,obj.hBeams,'UniformOutput',false);
                ...
                if ~obj.isVdaq
                    ...
                    obj.hTaskSoftPulse.createDOChan(obj.hTaskMain.deviceName,triggerChannel);
                    obj.softPulseTermString = ['/' obj.hTaskMain.deviceName '/PFI8'];
                end
    ...
    methods
        function set.stimTriggerTerm(obj, v)
            ...
            if obj.isVdaq
                ...
                obj.stimTrigIsFrame = strcmp(v,'frame');
            else
                obj.stimTrigIsFrame = ischar(v) && strcmp(v,'frame');
                ...
                if obj.numInstances
                    ...
                    obj.trigTermString = sprintf('/%s/PFI%d',obj.hTaskMain.deviceName,pfn);
        ...
        function set.autoTriggerPeriod(obj, v)
            ...
            if ~obj.isVdaq
                if obj.numInstances && v
                    ...
                    obj.hTaskAutoTrigger.createCOPulseChanTime(obj.hTaskMain.deviceName, 3, '', v/2, v/2, 0);
        ...
        function set.syncTriggerTerm(obj, v)
            ...
            if obj.isVdaq
                ...
                elseif obj.numInstances
                    ...
                    obj.syncTermString = sprintf('/%s/PFI%d',obj.hTaskMain.deviceName,pfn);
        ...
        %       photostimObj.start()
            ...
            try
                ...
                if ~obj.hScan.simulated
                    % set up triggering
                    if ~obj.isVdaq && ~isempty(obj.syncTriggerTerm) && ~most.idioms.isValidObj(obj.hTaskSyncHelper)
                    ...
                    if ~obj.isVdaq && ~most.idioms.isValidObj(obj.hTaskArmedTrig)
                    ...
                    if obj.isVdaq
                        ...
                        if obj.autoTriggerPeriod
                            trms = {sprintf('/%s/Ctr3InternalOutput',obj.hTaskMain.deviceName), obj.trigTermString};
                            obj.hRouteRegistry.connectTerms(trms{1}, trms{2});
                    ...
                    if obj.hasSlm && ~obj.hScan.active
                        ...
                        if strcmp(obj.stimulusMode, 'sequence')
                            ...
                            if obj.hSlm.hDevice.queueAvailable && most.idioms.isValidObj(obj.slmTriggerOutputChannel)
                                ...
                                % correct order
                                shuffledMasks = [masks(2:end);masks(1)];
                                shuffledMasks = cat(3,shuffledMasks{:});
                                shuffledMasks = gather(shuffledMasks);
                                ...
                                obj.hSlm.hDevice.writeQueue(shuffledMasks);
