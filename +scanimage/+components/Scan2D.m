classdef Scan2D < scanimage.interfaces.Component
    ...
    properties (SetObservable)
        ...
        parkSlmForAcquisition = true;       % park SLM during acquisition
    end
    
    properties (Abstract, Transient, SetObservable)
        hFastZs
        hShutters
        hBeams
    ...
    properties (SetObservable, Transient)
        ...
        physicalChannelsAvailable;          % number of channels available on the digitizer board
    ...
    properties (SetAccess = protected, SetObservable)
        scannerFrequency = 8000;                   % [Hz] frequency of resonant scanner
    end
...
%     end
    ...
    %% LIFECYCLE
    methods
        function obj = Scan2D(name)
            assert(isvarname(name),'Scanner name ''%s'' is invalid. Name must be usable as a Matlab variable name (e.g. MyScanner1)',name);
            obj@scanimage.interfaces.Component(name,true);
        end
    end
...
%                assert(mod(val,1)==0 && val >= 1,'logFramesPerFile must be inf or a positive integer');
            ...
            if obj.componentUpdateProperty('logFramesPerFile',val)
                if obj.logFramesPerFileLock && ~isempty(obj.hSI) && isvalid(obj.hSI)
                    val = obj.framesPerAcq; % Should be frames per slice if slice > 1
                    val = max(1,val);
                end
...
%             end
        ...
        function set.physicalChannelsAvailable(obj,val)
            obj.mdlDummySetProp(val,'physicalChannelsAvailable');
        end
        ...
        function val = get.physicalChannelsAvailable(obj)
            val = obj.accessPhysicalChannelsAvailablePostGet();
        end
        ...
        function set.scannerToRefTransform(obj, v)
            if scanimage.mroi.util.isTransformPerspective(v)
                most.idioms.warn('Ignoring perspective matrix entries in scanfield constraints.');
            end
            
            if obj.componentUpdateProperty('scannerToRefTransform',v)
                v = obj.validatePropArg('scannerToRefTransform',v);
                v = obj.accessScannerToRefTransformPreSet(v);
        ...
        function set.channelsSubtractOffsets(obj,val)
            ...
            if numel(val) < obj.physicalChannelsAvailable
                val(end+1:obj.physicalChannelsAvailable) = val(end);
            elseif numel(val) > obj.physicalChannelsAvailable
        ...
        function set.parkSlmForAcquisition(obj,val)
            validateattributes(val,{'numeric','logical'},{'binary','scalar'});
            obj.parkSlmForAcquisition = val;
        end
    end
    ...
    methods
        function activateBeams(obj,tf)
            validateattributes(tf,{'numeric','logical'},{'scalar','binary'});
            
            scanners = obj.hSI.hBeams.wrapBeams(obj.hBeams);
            powerFractions = scanners.powerFraction;
            
            for idx = 1:numel(obj.hBeams)
                powerFraction = powerFractions(idx) * tf;
                obj.hBeams{idx}.setPowerFraction(powerFraction);
            end
        end
        
        function openShutters(obj)
            obj.transitionShutters(true);
        end
        
        function closeShutters(obj)
            obj.transitionShutters(false);
        end
        
        function transitionShutters(obj,tf)
            obj.startShuttersTransition(tf);
            obj.waitShuttersTransitionComplete();
        end
        
        function startShuttersTransition(obj,tf)
            for idx = 1:numel(obj.hShutters)
                try
                    obj.hShutters{idx}.startTransition(tf);
                catch ME
        ...
        function waitShuttersTransitionComplete(obj)
            for idx = 1:numel(obj.hShutters)
                try
                    obj.hShutters{idx}.waitTransitionComplete();
                catch ME
                    most.ErrorHandler.logAndReportError(ME);
                end
            end
    ...
    methods (Abstract, Hidden)
        ...
        updateLiveValues(obj,regenAO,waveforms);    % request an update of the task ao
