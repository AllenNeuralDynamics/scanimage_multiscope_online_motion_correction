classdef AllenMotionCorrectorMultiZ < scanimage.interfaces.IMotionCorrector    
    ...
    methods
        function obj = AllenMotionCorrectorMultiZ()
        ...
        function drRef = getCorrection(obj)
            ...
            for zidx = 1:numel(s)
                ...
                correlations1 = mean(correlations1,1);
                correlations2 = mean(correlations2,1);

                [~,idx1] = max(correlations1);
                [~,idx2] = max(correlations2);

                % assume numel(correlationsZ_Ch1) is odd
                centerIdx1 = (numel(correlations1)-1)/2+1;
                centerIdx2 = (numel(correlations2)-1)/2+1;
