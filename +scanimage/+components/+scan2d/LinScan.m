classdef LinScan < scanimage.components.Scan2D & most.HasMachineDataFile & dabs.resources.configuration.HasConfigPage
    ...
    methods (Access = protected, Hidden)
        function val = accessScannersetPostGet(obj,~)
            ...
            assert(most.idioms.isValidObj(obj.yGalvo),'xGalvo is not defined in machine data file');
            ...
            for idx = 1:numel(beams)
                beams(idx).sampleRateHz = beamsSampleRate;
            ...
            val = scanimage.mroi.scannerset.GalvoGalvo(obj.name,xGalvoScanner,yGalvoScanner,beams,fastZScanners,...
