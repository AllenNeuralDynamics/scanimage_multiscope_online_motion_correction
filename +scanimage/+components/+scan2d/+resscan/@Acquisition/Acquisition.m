classdef Acquisition < scanimage.interfaces.Class
    ...
    properties (Hidden, Constant)
        ...
        FPGA_RAW_SAMPLE_RATE_TO_SAMPLE_RATE_FACTOR = containers.Map({'NI5732','NI5733','NI5734','NI5751','NI517x','NI5771','NI5771 (Time Demux)','NI5772'},{1,1,1,1,2,16,[],[]});
    ...
    methods
        function reinit(obj)
            ...
            if most.idioms.isValidObj(obj.hScan.hDAQAcq.hFpga)
                obj.hFpga = obj.hScan.hDAQAcq.hFpga;
            else
                obj.hFpga = obj.hScan.hDAQAcq.initFPGA();
            end
            ...
            if ~isempty(obj.FPGA_STATE_MACHINE_LOOP_RATE(obj.flexRioAdapterModuleName))
                ...
                obj.rawSampleRateAcq = obj.ADAPTER_MODULE_RAW_SAMPLING_RATE_MAP(obj.flexRioAdapterModuleName);
            end
        ...
        function initializeFpga(obj)
            %Initialize Mask
            obj.computeMask();
            
            %Initialize some fpga defaults
            period = obj.stateMachineLoopRate / obj.hScan.hResonantScanner.nominalFrequency_Hz;
    ...
    methods
        ...
        function val = get.auxTriggersEnable(obj)
           ...
           val= obj.hScan.auxTriggersEnable && ~strcmp(obj.flexRioAdapterModuleName,'NI5772'); 
        end
...
%             val = val * obj.interlacedNumChannels;
        ...
        function val = get.estimatedPeriodClockDelay(obj)
            ...
            linesPerTrigger = 2 ^ (~obj.hScan.isPolygonalScanner);
            totalTicksPerLine = (obj.stateMachineLoopRate / obj.hScan.hResonantScanner.nominalFrequency_Hz) / linesPerTrigger;
...
%             end
    ...
    methods (Hidden)
        ...
        function zprpResizeAcquisition(obj)
            ...
            assert((obj.hScan.mdfData.I2CEnable + obj.hScan.auxTriggersEnable + obj.hScan.mdfData.photonCountingEnable) <= 1,'I2CEnable, auxTriggersEnable, usePhotonCountinModule are mutually exclusive');
            if obj.hScan.mdfData.I2CEnable
                obj.hFpga.AuxDataFifoMode = 'I2C';
            elseif obj.hScan.auxTriggersEnable
                obj.hFpga.AuxDataFifoMode = 'Aux Triggers';
            else
                obj.hFpga.AuxDataFifoMode = 'Disabled';
            end                    
            
            if obj.hScan.auxTriggersEnable || obj.hScan.mdfData.I2CEnable
                if ~obj.simulated                   
...
%                 end
        ...
        function stripeData = frameToRois(obj,stripeData,frameData)            
            if isnan(stripeData.zIdx)
                ...
                startLines = obj.acqParamBuffer.startLines{stripeData.zIdx};
                endLines = obj.acqParamBuffer.endLines{stripeData.zIdx};
                
                rois = obj.acqParamBuffer.rois{stripeData.zIdx};
                for i = 1:numel(rois)
...
%                 stripeData.overvoltage = dabs.ni.oscope.checkOverload;
        ...
        function configureAdapterModuleSampleClock(obj,mode)
            ...
            function configureExternalSampleClock()
                ...
                assert(( min(obj.hScan.externalSampleClockRate) >= loopRateRange(1) ) && ...
                       ( max(obj.hScan.externalSampleClockRate) <= loopRateRange(2) ),...
                    ...
                    obj.hScan.externalSampleClockRate/1e6,obj.flexRioAdapterModuleName,loopRateRange(1)/1e6,loopRateRange(2)/1e6);
                
                if strcmp(obj.flexRioAdapterModuleName, 'NI517x')
                    dabs.ni.oscope.configureSampleClock(true,obj.hScan.externalSampleClockRate);
                else
                    command   = 0; % 0 = Clock Settings
                ...
                obj.rawSampleRateAcq = obj.hScan.externalSampleClockRate; %preliminary, actual loop rate measured later
            end
        end
        ...
        function measureExternalRawSampleClockRate(obj)
            ...
            function plotMeasurement(measurements)
                ...
                if isempty(hFig) || ~ishghandle(hFig)
                    hFig = most.idioms.figure('Name','FPGA digitizer module sample frequency','NumberTitle','off','MenuBar','none');
                end
...
%         end
        ...
        function bufferAcqParams(obj,live)
            ...
            for idx = 1:numel(uniqueScanFields)
                zScanfields = uniqueScanFields{idx};
                [uniqueStartLines{idx},uniqueEndLines{idx}] = acqActiveLines(scannerset,zScanfields);   
            end
            
            for idx = numel(zs) : -1 : 1
                z = zs(idx);
                zmask = z==uniqueZs;
                zScanfields = uniqueScanFields{zmask};
                scanFields{idx} = zScanfields;
        ...
        function resonantScannerFreq = calibrateResonantScannerFreq(obj,averageNumSamples)
            ...
            if ~obj.simulated
                if ~logical(obj.hFpga.AcqStatusPeriodClockSettled)
                    ...
                    if rejRate > (obj.hScan.hResonantScanner.nominalFrequency_Hz * .01)
    ...
    methods (Access = private)        
        function fpgaStartAcquisitionParameters(obj)
            ...
            if obj.hScan.mdfData.photonCountingEnable
                ...
                obj.hScan.auxTriggersEnable = false; % because FPGA PFI0..3 is used for photon counting
            ...
            if isprop(obj.hFpga, 'fifo_SampleScope')
                ...
                obj.hFpga.ResScanFilterSamples = obj.hScan.laserTriggerSampleMaskEnable;
                obj.hFpga.LaserTriggerDelay = obj.hScan.laserTriggerSampleWindow(1);
                obj.hFpga.LaserSampleWindowSize = obj.hScan.laserTriggerSampleWindow(2);
                obj.hFpga.LaserTriggerFilterTicks = obj.hScan.laserTriggerFilterTicks;
            end
            
            % configure aux triggers
            obj.hFpga.AuxTriggerEnable = obj.hScan.auxTriggersEnable;
        ...
        function fpgaModuleName = fpgaDetectAdapterModule(obj)
            ...
            if ~isempty(strfind(obj.hScan.hDAQAcq.productName, '517'))
                fpgaModuleName = 'NI517x';
                return;
            elseif obj.simulated
                % fake 5734 adapter module
