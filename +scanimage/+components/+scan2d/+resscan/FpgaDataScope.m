classdef FpgaDataScope < scanimage.components.scan2d.interfaces.DataScope
    ...
    %% LifeCycle
    methods
        function obj = FpgaDataScope(hScan2D,hNIRIO)
            obj.hScan2D = hScan2D;
            obj.hNIRIO = hNIRIO;
            
            if isa(hScan2D,'scanimage.components.scan2d.LinScan')
    ...
    methods
        ...
        function start(obj)
            ...
            assert(most.idioms.isValidObj(obj.hFpga),'No bitfile is loaded to FPGA');
    ...
    methods
        function val = get.hFpga(obj)
            val = obj.hNIRIO.hFpga();            
        end
        
        function val = get.hFifo(obj)
            val = obj.hFpga.fifo_DataScopeTargetToHostU32;
        end
