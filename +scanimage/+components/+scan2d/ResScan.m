classdef ResScan < scanimage.components.Scan2D & most.HasMachineDataFile & dabs.resources.configuration.HasConfigPage
    ...
    methods
        ...
        function val = get.isPolygonalScanner(obj)
            val = most.idioms.isValidObj(obj.hResonantScanner) ...
               && isa(obj.hResonantScanner,'dabs.resources.devices.PolygonalScanner');
        end
    ...
    methods (Access = protected, Hidden)
        function val = accessScannersetPostGet(obj,~)
            ...
            for idx = 1:numel(beams)
                beams(idx).sampleRateHz = obj.hSI.hBeams.maxSampleRate;
                ...
                beams(idx).includeFlybackLines = false;
            ...
            val=scanimage.mroi.scannerset.ResonantGalvoGalvo(obj.name,r,xGalvoScanner,yGalvoScanner,beams,fastZScanners,obj.fillFractionSpatial);
    ...
    methods (Access = protected, Hidden)
        ...
        function componentStart(obj)
            ...
            for idx = 1:numel(obj.hBeams)
                obj.hBeams{idx}.setLastKnownPowerFractionToNaN();
            end
