        ...
        function checkFifo(obj,varargin)
            ...
            if obj.isH
                ...
                if obj.includeSyncTrigger
                    syncTrigData = u16Data(26:pCache.u16Columns:end);
                    
                    darray = zeros(pCache.numPackets*16,1,'uint16');
                    for i = 1:pCache.numPackets
                        darray((i*16-15):(i*16)) = bitand(syncTrigData(i), pCache.dmask);
                    end
                    
                    triggers.SyncTrigger = reshape(repmat(logical(darray),1,2)',[],1);
                end
            else
            ...
            function s = triggerDecode(trigger)
                ...
                s.AcquisitionActive     = getDig(trigger,8);
