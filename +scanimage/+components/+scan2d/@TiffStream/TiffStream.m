classdef TiffStream < handle
    ...
    methods (Access = public)
        ...
        function boolResult = configureImage(obj, imageWidth, imageLength, bytesPerPixel, numberOfChannels, signedData, imageDescription,...
                xResolutionNumerator, xResolutionDenominator, yResolutionNumerator, yResolutionDenominator, targetBytesPerFullStrip)
        ...
        % 
        % Calling Sequence:
        %   boolResult = obj.configureImage(imageWidth, imageLength, bytesPerPixel, numberOfChannels, signedData, imageDescription, targetBytesPerFullStrip)
        % 
        % Define Variables:
        ...
        %       the parameter 'targetBytesPerFullStrip' is used. The value can
        %       be NaN.
        %   targetBytesPerFullStrip -- unsigned int -- Optional -- The
        ...
        %               
            ...
            if (nargin == 12)
                res = obj.ConfigureImageMex(uint64(obj.twhandle), uint16(imageWidth), uint16(imageLength), uint16(bytesPerPixel), uint16(numberOfChannels), signedData, char(imageDescription),...
                    uint32(xResolutionNumerator), uint32(xResolutionDenominator), uint32(yResolutionNumerator), uint32(yResolutionDenominator), uint32(targetBytesPerFullStrip));
            elseif (nargin == 11)
                res = obj.ConfigureImageMex(uint64(obj.twhandle), uint16(imageWidth), uint16(imageLength), uint16(bytesPerPixel), uint16(numberOfChannels), signedData, char(imageDescription),...
