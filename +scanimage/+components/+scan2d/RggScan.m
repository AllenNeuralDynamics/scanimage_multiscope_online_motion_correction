classdef RggScan < scanimage.components.Scan2D & most.HasMachineDataFile & dabs.resources.configuration.HasConfigPage
    ...
    methods
        ...
        function set.hResonantScanner(obj,val)
            ...
            if ~isequal(val,obj.hResonantScanner)
                if most.idioms.isValidObj(val)
                    validateattributes(val,{'dabs.resources.devices.ResonantScanner'},{'scalar'});
                end
        ...
        function set.hBeams(obj,val)
            ...
            for idx = 1:numel(val)
                validateattributes(val{idx},{'dabs.resources.devices.BeamModulatorFast'},{'scalar'});
            end
        ...
        function set.sampleRate(obj,val)
            if obj.scanModeIsResonant && ~isnan(val)
                ...
                obj.sampleRateDecim = max(1,2*floor(0.5*obj.hAcq.stateMachineLoopRate/val));
            end
            if obj.mdlInitialized
                obj.sampleRateCtl = nan;
            end
        ...
        function val = get.sampleRate(obj)
            if obj.scanModeIsResonant
                val = obj.hAcq.acqSampleRate;
            else
                val = obj.hAcq.stateMachineLoopRate / obj.sampleRateDecim;
            end
        end
        ...
        function v = get.ctlTimebaseRate(obj)
            if obj.scanModeIsLinear && obj.hCtl.useScannerSampleClk
                v = obj.hAcq.stateMachineLoopRate;
            else
        ...
        function val = get.validSampleRates(obj)
            if obj.scanModeIsResonant
                ...
                val = obj.hAcq.acqSampleRate ./ (6:2:1200);
            end
        end
        ...
        function v = defaultVirtChanSettings(obj)
            if isempty(obj.hAcq) || isempty(obj.hAcq.hFpga) || isempty(obj.hAcq.hFpga.hAfe)
                ...
                isH = strcmp(obj.hAcq.hFpga.hAfe.moduleType,'H');
            end
        ...
        function set.virtualChannelSettings(obj,v)
            ...
            if isempty(v)
                ...
                validationFuncs.source = @(v)assert(ismember(lower(v),arrayfun(@(i){sprintf('ai%d',i-1)},1:obj.physicalChannelsAvailable)));
                ...
                validationFuncs.laserGate = @(v)validateattributes(v,{'numeric' 'logical'},{'scalar'});
        ...
        function v = get.MAX_NUM_CHANNELS(obj)
            v = obj.hAcq.maxNumVirtualChannels;
        end
    end
    ...
    methods (Access = protected, Hidden)
        ...
        function val = accessScanPixelTimeMeanPostGet(obj,~)
            ...
            if obj.scanModeIsResonant
                val = (sum(obj.hAcq.mask(1:obj.hAcq.pixelsPerLine)) / obj.hAcq.stateMachineLoopRate) / obj.hAcq.pixelsPerLine;
            else
    ...
    methods
        ...
        function centerScanner(obj)
            ...
            if obj.componentExecuteFunction('centerScanner')
                if obj.hasResonantMirror
                    obj.hResonantScanner.setAmplitude(0);
                end
    ...
    methods        
        function resFreq = measureScannerFrequency(obj)
            ...
            if obj.componentExecuteFunction('measureScannerFrequency')
                ...
                fprintf('Measuring scanner frequency at amplitude %.3fdeg (peak-peak)...\n',amplitude);
    ...
    methods (Hidden)
        function ss = getScannerset(obj, enableResonant, forStim)
            ...
            beams = obj.hSI.hBeams.wrapBeams(obj.hBeams);
            for idx = 1:numel(beams)
                beams(idx).sampleRateHz = ctlFs;
                beams(idx).linePhase = obj.linePhase;
                beams(idx).beamClockDelay = obj.beamClockDelay;
                beams(idx).beamClockExtend = obj.beamClockExtend;
                beams(idx).includeFlybackLines = true;
                
                if obj.hSI.hRoiManager.isLineScan
                    beams(idx).powerBoxes = [];
                end
            end
            ...
            if enableResonant
                ...
                ss=scanimage.mroi.scannerset.ResonantGalvoGalvo(obj.name,r,xGalvoScanner,yGalvoScanner,beams,fastZScanners,obj.fillFractionSpatial);
                ...
                stepY = false; % ????????
                ss = scanimage.mroi.scannerset.GalvoGalvo(obj.name,xGalvoScanner,yGalvoScanner,beams,fastZScanners,...
    ...
    methods (Access = protected, Hidden)
        function mdlInitialize(obj)
            ...
            function s = defaultModeProps()
                ...
                s.linear = struct('sampleRate', lsr, 'pixelBinFactor', 8, 'fillFractionSpatial', .9, 'bidirectional', true, 'stripingEnable', true,...
                    'linePhase', 0, 'flybackTimePerFrame', obj.defaultFlybackTimePerFrame, 'flytoTimePerScanfield', obj.defaultFlytoTimePerScanfield);
                s.resonant = struct('uniformSampling', false, 'pixelBinFactor', 1, 'fillFractionSpatial', .9, 'bidirectional', true, 'stripingEnable', false,...
                    'linePhase', 0, 'flybackTimePerFrame', obj.defaultFlybackTimePerFrame, 'flytoTimePerScanfield', obj.defaultFlytoTimePerScanfield);
            end
        end
