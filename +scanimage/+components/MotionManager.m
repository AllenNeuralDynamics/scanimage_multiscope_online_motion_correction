classdef MotionManager < scanimage.interfaces.Component & most.HasClassDataFile
    ...
    properties (SetObservable, SetAccess = private, Transient)
        ...
        motionCorrectionVector = zeros(1,4); % Currently applied absolute motion correction (in reference space coordinates)
    end
    ...
    methods (Access = ?scanimage.SI)
        function obj = MotionManager()
            obj@scanimage.interfaces.Component('SI MotionManager');
        end
    end
    ...
    methods        
        ...
        function resetMotionCorrection(obj,moveAxes)
            ...
            if moveAxes && obj.active && obj.enable
                obj.moveAxesAbsolute(zeros(1,4));
            else
                obj.motionCorrectionVector = zeros(1,4);
            end
        end
        ...
        function manualCorrectXY(obj,dr)
            ...
            if correctionXYPossible
                forceXY = true;
                forceZ  = false;
                obj.correctMotion(dr,forceXY,forceZ);
            else
                msg = sprintf('XY Correction is not possible with the currently selected actuator.\nIf using motors, make sure to perform motor alignment first.');
                warndlg(msg,'Motion Correction');
...
%                 obj.setClassDataVar('lastFile',tifPath);
    ...
    methods (Hidden)
        ...
        function correctMotion(obj,dr,forceXY,forceZ)
            ...
            validateattributes(dr,{'numeric'},{'vector','numel',4});
            ...
            function dAbsNew = coerceCorrectionAxis(axis_number,dAbsOld,d)
                correctionEnableAxes = [obj.correctionEnableXY obj.correctionEnableXY obj.correctionEnableZ obj.correctionEnableZ];
                force = [forceXY forceXY forceZ forceZ];
    ...
    methods (Access = private)
        ...
        function moveAxesAbsolute(obj,newMotionCorrectionVector)
            ...
            validateattributes(newMotionCorrectionVector,{'numeric'},{'numel',4,'nonnan','finite'});
            ...
            if moveFastZStatic
                for fastZIdx = 1:numel(obj.hSI.hScan2D.hFastZs)
                    hFastZ_ = obj.hSI.hScan2D.hFastZs{fastZIdx};
                    hFastZ_.move( hFastZ_.targetPosition + fastZ_static_offset );
                end
            end
            ...
            function [moveGalvos,galvo_volt_offset] = getGalvoVoltCorrectionXY()
                ...
                if correctionXYPossible && useGalvos
                    ...
                    x_ao_max_deg = sort(x_gscanner.hDevice.volts2Position(obj.performanceCache.ao_GX_max_volts));
                    y_ao_max_deg = sort(y_gscanner.hDevice.volts2Position(obj.performanceCache.ao_GY_max_volts));
                    
                    x_allowed_d = x_gscanner.hDevice.travelRange-x_ao_max_deg;
                    y_allowed_d = y_gscanner.hDevice.travelRange-y_ao_max_deg;
                    ...
                    if ~isequal(newMotionCorrectionVector(1:2),obj.motionCorrectionVector(1:2))
                        galvo_volt_offset(1) = x_gscanner.hDevice.position2Volts(d_galvo_deg(1))-x_gscanner.hDevice.position2Volts(0);
                        galvo_volt_offset(2) = y_gscanner.hDevice.position2Volts(d_galvo_deg(2))-y_gscanner.hDevice.position2Volts(0);
                        moveGalvos = true;
                    end
                end
            end
            ...
            function [moveFastZDynamic,fastZ_volt_offset,moveFastZStatic,fastZ_static_offset] = getFastZCorrection()
                ...
                useFastZ = any(dr(3:4)) && strcmpi(obj.correctionDeviceZ,'fastz');
                if correctionZPossible && useFastZ
                    if obj.hSI.hFastZ.outputActive
                        % move FastZ dynamically by changing output waveform
                        hFastZs = {ss.fastz.hDevice};
                       
                        
                        %%% coerce fastZ to allowed actuator voltage range
                        for idx = 1:numel(hFastZs)
                            dIdx = 3+idx-1;
                            dz = newMotionCorrectionVector(dIdx);

                            hFastZ = hFastZs{idx};
                            aoz_ref = sort( hFastZ.volts2Position([obj.performanceCache.ao_Z_min_volts(idx) obj.performanceCache.ao_Z_max_volts(idx)]) );
                            aoz_min_ref = aoz_ref(1);
                            aoz_max_ref = aoz_ref(2);
                            
                            z_lims_ref  = sort( hFastZ.volts2Position(hFastZ.position2Volts(hFastZ.travelRange)) );
                            z_lim_min_ref = z_lims_ref(1);
                            z_lim_max_ref = z_lims_ref(2);

                            allowedCorrectionMin = max( z_lim_min_ref - aoz_min_ref );
                            allowedCorrectionMax = min( z_lim_max_ref - aoz_max_ref );

                            dz = max(min(dz,allowedCorrectionMax),allowedCorrectionMin); % coerce correction range

                            newMotionCorrectionVector(dIdx) = dz;
                            dr(dIdx) = newMotionCorrectionVector(dIdx)-obj.motionCorrectionVector(dIdx);
                        end

                        fastZ_volt_offset = zeros(1,numel(hFastZs));
                        ...
                        for idx = 1:numel(hFastZs)
                            ...
                            fastZ_volt_offset(idx) = hFastZ.position2Volts(dz)-hFastZ.position2Volts(0);

                            if ~isequal(newMotionCorrectionVector(dIdx),obj.motionCorrectionVector(dIdx))
                        ...
                        for idx = 1:numel(hFastZs)
                            ...
                            pos = hFastZ.targetPosition + dr(3);
                            pos = min(max(pos,travelRange(1)),travelRange(2));
                            dz(idx) = pos - hFastZ.targetPosition;
                        end
    ...
    methods(Hidden, Access = protected)
        function componentStart(obj)
            ...
            function s = bufferScannerAOMinMax(s)
                ...
                if ao_Z_valid
                    s.ao_Z_min_volts = min(ao_Z);
                    s.ao_Z_max_volts = max(ao_Z);
                end
            end
