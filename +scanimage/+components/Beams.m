classdef Beams < scanimage.interfaces.Component
    ...
    properties (SetObservable)
        ...
        powerBoxes = struct('rect', [.25 .25 .5 .5], 'powers', NaN, 'name', '','oddLines',true,'evenLines',true,'mask',[]); % Array of structures containing parameters for each power box.
    ...
    methods (Hidden)        
        ...
        function searchForBeams(obj)
            hBeams_       = obj.hResourceStore.filterByClass('dabs.resources.devices.BeamModulatorFast');
            ...
            if ~isequal(obj.hBeams,hBeams_) || ~isequal(obj.hBeamRouters,hBeamRouters_) 
                ...
                obj.numInstances  = numel(obj.hBeams);
                obj.totalNumBeams = numel(obj.hBeams);
    ...
    methods
        ...
        function v = get.currentScanners(obj)
            v = obj.wrapBeams(obj.currentBeams);
        end
        ...
        function scanners = wrapBeams(obj,hBeams)
            scanners = scanimage.mroi.scanners.Beams.empty();
            ...
            for idx = 1:numel(hBeams)
                ...
                scanner = scanimage.mroi.scanners.Beams(hBeam);
                ...
                scanner.powerBoxes = getPowerBoxes(idx);
            ...
            function pbxs = getPowerBoxes(beamIdx)
                if obj.enablePowerBox
                    pbxs = obj.powerBoxes;
                    for pbxIdx = 1:numel(pbxs)
                        numDefinedPowers = numel(pbxs(pbxIdx).powers(beamIdx));
                        if numDefinedPowers == 1
                            pbxs(pbxIdx).powers = pbxs(pbxIdx).powers(1);
                        elseif numDefinedPowers >= pbxIdx
                            pbxs(pbxIdx).powers = pbxs(pbxIdx).powers(pbxIdx);
                        else
                            pbxs(pbxIdx).powers = NaN;
                        end
                    end                    
                else
                    pbxs = [];
    ...
    methods (Hidden)        
        function updateSliceAO(obj)
            ...
            if ~isempty(obj.currentScanners) && ~obj.sharingScannerDaq
        ...
        function recreateBeamDaqTask(obj)
            ...
            scanners_ = obj.currentScanners;
            
            if isempty(scanners_)
                obj.sharingScannerDaq = false;
                return
            end
            
            arrayfun(@(s)s.hDevice.assertNoError(),scanners_);
            
            hDAQs = arrayfun(@(s)s.hDevice.hAOControl.hDAQ,scanners_,'UniformOutput',false);
            hDAQ = hDAQs{1};
            ...
            for idx = 1:numel(scanners_)
                scanner = scanners_(idx);
    ...
    methods
        function deactivateCurrentBeams(obj)
            for idx = 1:numel(obj.currentBeams)
                try
                    obj.currentBeams{idx}.setPowerFraction(0);
                catch ME
                    most.ErrorHandler.logAndReportError(ME);
                end
            end
