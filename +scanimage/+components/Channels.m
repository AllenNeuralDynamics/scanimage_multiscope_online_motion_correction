classdef Channels < scanimage.interfaces.Component
    ...
    methods (Access = ?scanimage.SI)
        function obj = Channels()
            obj@scanimage.interfaces.Component('SI Channels');
        end
    end
    ...
    methods
        ...
        function set.channelMergeColor(obj,val)
            ...
            if obj.componentUpdateProperty('channelMergeColor',val)
                val = obj.ensureChannelPropSize(val,[],'virtualmax');
        ...
        function set.channelDisplay(obj,val)
            ...
            val(val>obj.channelsAvailable) = [];
            
            if obj.mdlInitialized && isempty(union(val,obj.channelSave))
        ...
        function set.channelSave(obj,val)
            ...
            if obj.mdlInitialized && isempty(union(val,obj.channelDisplay))
                most.idioms.warn('At least one channel must be selected for display or logging');
            end
        ...
        function set.channelOffset(obj,val)
            if ~isempty(obj.hSI.hScan2D) && ~any(isnan(val))
                ...
                if obj.componentUpdateProperty('channelOffset',val)
                    val = obj.ensureChannelPropSize(val,'row','physical');
                    obj.hSI.hScan2D.channelOffsets = val;
                end
            end
        end
        ...
        function set.channelInputRange(obj,val)
            if ~isempty(obj.hSI.hScan2D) && (iscell(val) || ~any(isnan(val)))
                ...
                if obj.componentUpdateProperty('channelInputRange',val)
                    val = obj.ensureChannelPropSize(val,'row','physical');
                    obj.hSI.hScan2D.channelsInputRanges = val;
                end
            end
        end
        ...
        function set.channelSubtractOffset(obj,val)
            if ~isempty(obj.hSI.hScan2D) && ~any(isnan(val))
                ...
                if obj.componentUpdateProperty('channelSubtractOffset',val)
                    val = obj.ensureChannelPropSize(val,'row','physical');
                    obj.hSI.hScan2D.channelsSubtractOffsets = logical(val);
                end
            end
        end
    ...
    %% USER METHODS
    methods
        function registerChannels(obj,channelCountUpdateOnly)   
            obj.channelsAvailable = obj.hSI.hScan2D.channelsAvailable;
            
            if (nargin > 1) && channelCountUpdateOnly
                obj.channelSave = obj.channelSave;
                obj.channelDisplay = obj.channelDisplay;
                return;
            end
            ...
            if ~isfield(obj.imagingSystemSettings, obj.hSI.imagingSystem)
                ...
                for iter = 1:obj.hSI.hScan2D.MAX_NUM_CHANNELS
                    ...
                    if numel(obj.hSI.hScan2D.channelsInputRanges) >= iter
    ...
    methods (Access = private)        
        function val = ensureChannelPropSize(obj,val,orientation,channelType)
            ...
            if nargin < 4 || isempty(channelType)
                channelType = 'virtual';
            end
            ...
            switch lower(channelType)
                ...
                case 'virtualmax'
                    numChans = obj.hSI.hScan2D.MAX_NUM_CHANNELS;
                otherwise
                    numChans = obj.hSI.hScan2D.physicalChannelsAvailable;
            end
