classdef AcquisitionEngine < dabs.vidrio.rdi.Device
    ...
    methods
        ...
        function v = get.acqParamDisableDivide(obj)
            ...
            mask = cast(2.^(0:min(obj.MAX_NUM_LOGICAL_CHANNELS-1,31)),'like',v);
            v = logical(bitand(v,mask));
            v(min(end+1,obj.NUM_DIVIDERS+1):obj.MAX_NUM_LOGICAL_CHANNELS) = true;
        end
...
function s = initRegs()
    ...
    s.dataRegs.maxNumVirtualChannels = struct('address',8,'hide',true);
    s.dataRegs.hsadcSupport = struct('address',8,'hide',true);
    s.dataRegs.data = struct('address',8,'hide',true);
    
    s.dataRegs.MAX_NUM_LOGICAL_CHANNELS = struct('address',8,'hide',true);
