classdef vDAQ_SI < dabs.vidrio.rdi.Device
    ...
    methods
        ...
        function s = getRegMap(obj)
            ...
            if obj.isR1
                ...
                s.dataRegs.hsSyncTrigDelay = struct('address',4194304+180);
                s.dataRegs.hsSyncTrigLockWaveform = struct('address',4194304+184);
                s.dataRegs.hsSyncTrigInvert = struct('address',4194304+188);
            else
    ...
    methods
            ...
            if ~obj.initialized
                ...
                if obj.isR1
                    ...
                    if obj.simulate || (obj.NUM_AE > 1)
                        ...
                        % current bug in the FPGA where even if there is only one AE, the
                        % DO vector still includes space for the second AE
                        obj.spclOutputSignals = [obj.spclOutputSignals cellfun(@(~){'-'},obj.DIO_SPCL_OUTPUTS)];
                    end
        ...
        function configureHsadc(obj, clockSource, sourceClockRate, desiredSampleClockRate, passive)
            ...
            if ~passive
                ...
                obj.syncTrigDelayAdjust = obj.syncTrigDelayAdjust;
                obj.hsSyncTrigLockWaveform = (desiredSampleClockRate / sourceClockRate) == 32;
                obj.hsSyncTrigInvert = 1;
            else
                obj.meanBufferDelay = mean(obj.hHsadc.jesdBufferDelay);
            end
    ...
    methods
        ...
        function set.syncTrigDelayAdjust(obj,val)
            obj.syncTrigDelayAdjust = val;
            obj.hsSyncTrigDelay = round((106 + obj.meanBufferDelay*2.4375)/2) + obj.syncTrigDelayAdjust;
        end
    end
