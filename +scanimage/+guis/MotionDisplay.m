classdef MotionDisplay < most.Gui
    ...
    methods (Access = protected)
        ...
        function initLiveViewPanel(obj,hParent)
            ...
            obj.hLiveViewAxes = most.idioms.axes('Parent',hParent,'DataAspectRatio',[1 1 1],'box','on','Units','pixel','XLimMode','manual','XLimMode','manual','XLim',[-10 10],'YLim',[-10 10],'ButtonDownFcn',@(varargin)obj.moveLiveViewAxes,'Color','black','GridColor',most.constants.Colors.lightGray,'XGrid','on','YGrid','on');
            ...
            obj.hZCursorAxes = most.idioms.axes('Parent',hParent,'box','on','Units','pixel','XLim',[0 1],'YLim',[-100 100],'Visible','off');
            view(obj.hZCursorAxes,0,-90);
            obj.hZAxes = most.idioms.axes('Parent',hParent,'box','on','Units','pixel','XLim',[0 1],'YLim',[-100 100],'ButtonDownFcn',@(varargin)obj.moveZAxes,'YAxisLocation','right','XTick',[],'YTick',[],'Color','black');
            obj.hZAxesTicks = most.idioms.axes('Parent',hParent,'box','on','Units','pixel','XLim',[0 1],'YLim',[-100 100],'HitTest','off','YAxisLocation','right','XTick',[],'Color','none');
            ...
            obj.hZAxesContextMenu = uicontextmenu('Parent',obj.hFig);
                obj.hMenuAutoScaleZCorrelation = uimenu('Parent',obj.hZAxesContextMenu,'Label','Autoscale plot','Callback',@(varargin)obj.toggleAutoScaleZCorrelation);
                
            obj.hZAxes.UIContextMenu = obj.hZAxesContextMenu;
            obj.updateZContextMenu();
        ...
        function initPlotsPanel(obj,hParent)
            ...
            obj.hXPlotAxes = most.idioms.axes('Parent',hParent,'Units','pixel','box','on','xgrid','on','ygrid','on','XLim',[-1 0],'YLim',[-1 1]);
            title(obj.hXPlotAxes,'X motion [deg]','FontWeight','normal');
            obj.hYPlotAxes = most.idioms.axes('Parent',hParent,'Units','pixel','box','on','xgrid','on','ygrid','on','XLim',[-1 0],'YLim',[-1 1]);
            ...
            obj.hZPlotAxes = most.idioms.axes('Parent',hParent,'Units','pixel','box','on','xgrid','on','ygrid','on','XLim',[-1 0],'YLim',[-1 1]);
        ...
        function initCorrectionPlotsPanel(obj,hParent)
            ...
            obj.hXYCorrectionAxes = most.idioms.axes('Parent',hParent,'Units','pixel','box','on','DataAspectRatio',[1 1 1],'XLim',[-1 1],'YLim',[-1 1]);
            ...
            obj.hZCorrectionAxes = most.idioms.axes('Parent',hParent,'Units','pixel','box','on','XLim',[0 1],'YLim',[-1 1],'XTick',[],'YAxisLocation','right');
    ...
    methods
        ...
        function updatePlots(obj)
            ...
            if motionHistoryIsEmpty || isempty(obj.sfDisp) || isempty(obj.sfDisp.estimatorUuiduint64s)
                ...
                if any(mask)
                    ...
                    if obj.autoScaleZCorrelation || obj.maxZCorrelation<=0
                        obj.maxZCorrelation = maxCorrelation;
                    end
                    ...
                    for idx = 1:numel(correlations)
                        correlations{idx} = normalizeCorrelation(correlations{idx},obj.maxZCorrelation);
                    end
        ...
        function hSurf = checkOutSurface(obj)
            if isempty(obj.surfStorage)
                hParent = obj.hLiveViewAxes;
                assert(most.idioms.isValidObj(hParent));
                hSurf = surface('Parent',hParent,'CData',NaN);
        ...
        function updateZContextMenu(obj)
            obj.hMenuAutoScaleZCorrelation.Checked = tfToOnOff(obj.autoScaleZCorrelation);
            
            function v = tfToOnOff(v)
                if v
                    v = 'on';
                else
                    v = 'off';
        ...
        function toggleAutoScaleZCorrelation(obj)
            obj.autoScaleZCorrelation = ~obj.autoScaleZCorrelation;
        end
    end
    ...
    methods
        ...
        function set.autoScaleZCorrelation(obj,v)
            validateattributes(v,{'numeric','logical'},{'scalar','binary'});            
            obj.autoScaleZCorrelation = logical(v);
            
            obj.updateZContextMenu();
        end
        
        function set.maxZCorrelation(obj,v)
            if ~( isscalar(v) && v>0 )
                % Invalid maxZCorrelation, set to 0
                v = 0;
            end
            
            obj.maxZCorrelation = v;
        end
    end
end
