classdef LineScanDisplay < handle
    ...
    methods
        function obj = LineScanDisplay(hSI,hFig,chan)
            ...
            obj.hFig = ancestor(hFig,'figure');
            
            obj.historyLen = hSI.hDisplay.lineScanHistoryLength;
            obj.channel = chan;
            
            rs = [obj.hSI.hRoiManager.currentRoiGroup.rois];
            if ~isempty(rs)
                sfzs = [rs.zs];
                sfs = [rs.scanfields];
                unzs = unique(sfzs + [sfs.zSpan]);
            else
                unzs = 0;
            end
            
            obj.is3d = (numel(unzs) > 1) && obj.hSI.hFastZ.enable;
            
            if ~isempty(unzs)
                obj.zrng = [min(unzs) max(unzs)];
            ...
            obj.hTimePlotAx = most.idioms.axes('parent',hFig,'color','w','box','on','ylim',obj.CLim,'xlim',[1 obj.historyLen],'YTick',[],'YTickLabel',[],'UIContextMenu',hMenu);
            grid(obj.hTimePlotAx, 'on');
            xlabel(obj.hTimePlotAx,'Frame Number');
            ylabel(obj.hTimePlotAx,'Pixel Value');
            
            obj.hDataViewAx = most.idioms.axes('parent',hFig,'color','k','box','on','YDir','reverse','XTick',[],'XTickLabel',[],'xlim',[1 obj.historyLen],'ylim',[0 1],'YTick',[],'YTickLabel',[],'clim',[0 1],'UIContextMenu',hMenu);
            ...
            rg = [-.5 .5] * max(obj.hSI.hRoiManager.refAngularRange);
            obj.hSpacialViewPnl = uipanel('parent',hFig,'BorderType','None','backgroundcolor','k');
            obj.hSpacViewMouseFindAxes = most.idioms.axes('parent',obj.hSpacialViewPnl,'color','none','XColor','none','YColor','none','position',[0 0 1 1],'hittest','off');
            if obj.is3d
                obj.hSpacialViewAx = most.idioms.axes('parent',obj.hSpacialViewPnl,'color','k','PlotBoxAspectRatio', ones(1,3),'XColor','w','YColor','w','ZColor','w',...
                    'Projection','perspective','zlim',obj.zrng,'box','on','DataAspectRatio',[1 1 1],'XLim',rg,'YLim',rg,'UIContextMenu',hMenu);
                ...
                obj.hSpacialViewAx = most.idioms.axes('parent',obj.hSpacialViewPnl,'position',[0 0 1 1],'color','k','XTick',[],'XTickLabel',[],'YTick',[],...
                    'YTickLabel',[],'DataAspectRatio',[1 1 1],'XLim',rg,'YLim',rg,'XColor','none','YColor','none','UIContextMenu',hMenu);
