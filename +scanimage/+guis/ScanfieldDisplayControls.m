classdef ScanfieldDisplayControls < most.Gui
    ...
    properties (Hidden)
        ...
        dnChar =  ['<html><table border=0 width=16><TR><TD><center>' most.constants.Unicode.downwards_arrow  '</center></TD></TR></table></html>'];
        delChar = ['<html><table border=0 width=20><TR><TD><center>' most.constants.Unicode.ballot_x '</center></TD></TR></table></html>'];
    end
    ...
    methods
        function obj = ScanfieldDisplayControls(hModel, hController)
            ...
            obj.utSfTable = uitable(...
                ...
                'Tag','utSfTable');
            obj.hTblLstener = most.ErrorHandler.addCatchingListener(obj.hModel.hDisplay, 'scanfieldDisplays', 'PostSet', @obj.updateTable);
...
%%%                            'Bindings',{{obj.hModel.hDisplay 'scanfieldDisplayRows' 'Value'} {obj.hModel.hDisplay 'scanfieldDisplayRows' 'Callback' @(varargin)obj.resetDisplay()}},...
            ...
            obj.hRstLstener = most.ErrorHandler.addCatchingListener(obj.hModel.hDisplay, 'displayReset', @(varargin)obj.resetDisplay());
            
            %draw new frame
            obj.hDispListener = most.ErrorHandler.addCatchingListener(obj.hModel.hUserFunctions, 'frameAcquired', @(varargin)obj.displayUpdate());
            
            %ave factor changed
            obj.hAveListener = most.ErrorHandler.addCatchingListener(obj.hModel.hDisplay, 'displayRollingAverageFactor', 'PostSet', @(varargin)obj.updateLuts());
            
            %luts changed
            obj.hLutList = [most.ErrorHandler.addCatchingListener(obj.hModel.hDisplay, 'chan1LUT', 'PostSet', @(varargin)obj.updateLuts())...
                most.ErrorHandler.addCatchingListener(obj.hModel.hDisplay, 'chan2LUT', 'PostSet', @(varargin)obj.updateLuts())...
                most.ErrorHandler.addCatchingListener(obj.hModel.hDisplay, 'chan3LUT', 'PostSet', @(varargin)obj.updateLuts())...
                most.ErrorHandler.addCatchingListener(obj.hModel.hDisplay, 'chan4LUT', 'PostSet', @(varargin)obj.updateLuts())];
    ...
    methods (Hidden)
        function updateColorMaps(obj)
            if most.idioms.isValidObj(obj.hDisplayFig)
                ...
                cms = hCIN.hChannelControlsCAT.getColormaps();
                if ~isempty(cms)
                    cmf = cms{1};
        ...
        function resetDisplay(obj)
            ...
            if isempty(sgl)
                try
                    ...
                    fc = .05*ones(3,1);
                    if ~most.idioms.isValidObj(obj.hDisplayFig)
                        obj.hDisplayFig = most.idioms.figure('Name','Scanfield Display','Visible','off','NumberTitle','off','SizeChangedFcn',@(varargin)obj.resetDisplay(),...
...
%                             cols = ceil(N/rows);
                    ...
                    for i = 0:N-1
                        ...
                        if (i+1) > numel(obj.hDisplayAxes) || ~most.idioms.isValidObj(obj.hDisplayAxes(i+1))
                            obj.hDisplayAxes(i+1) = most.idioms.axes('Parent',obj.hDisplayFig,'units','pixels','Position',frmP,'YDir','reverse','XTick',[],...
