classdef  BeamControls < most.Gui    
    ...
    methods
        ...
        function val = get.scanner(obj)
            if isempty(obj.hModel.hBeams.hBeams)
                ...
                hDevices = {scanners.hDevice};
                currentBeam = obj.hModel.hBeams.hBeams{obj.beamIdx};
                mask = cellfun(@(hD)isequal(hD,currentBeam),hDevices);
                val = scanners(mask);
            end
        end
    ...
    methods
        ...
        function redrawLUTPlot(obj,varargin)            
            ...
            scanner_ = obj.hModel.hBeams.scanners(obj.beamIdx);
            ...
            zsInUseY = scanner_.powerDepthCorrectionFunc(scanner_.powerFraction,zsInUseX)*100;
            
            allXs = unique([lutDataX(:);zsInUseX(:)]);
            allYs = scanner_.powerDepthCorrectionFunc(scanner_.powerFraction,allXs)*100;
            ...
            if scanner_.powerFractionLimit < 1
                obj.hLnLutLimit.XData = xLim;
                obj.hLnLutLimit.YData = [100 100] * scanner_.powerFractionLimit;
            else
