classdef SignalConditioningControls < most.Gui
    ...
    methods (Hidden)
        ...
        function phyTableCb(obj,~,evt)
            switch evt.Indices(2)
                ...
                case 6
                    try
                        obj.hScan2D.channelsFilter = evt.NewData;
                    catch
                    end
                    obj.updatePhyTable();
                case 7
        ...
        function updatePhyTable(obj,varargin)
            if isempty(obj.hScan2D)
                ...
                if obj.isH
                    ...
                    f = obj.hScan2D.channelsFilter;
                    if strcmp(f,'fbw')
                        filters = repmat({'Bypass'},1,N);
                    else
                        filters = repmat({f},1,N);
                    end
                    d = arrayfun(@(n,r,i,s,o,f){{sprintf(' AI%d',n) sprintf('[%s %s]', num2str(r{1}(1)), num2str(r{1}(2))) logical(i) logical(s) o f{1}}'},0:(N-1),...
                        obj.hSI.hChannels.channelInputRange,obj.hScan2D.channelsInvert,obj.hSI.hChannels.channelSubtractOffset,obj.hSI.hChannels.channelOffset,filters);
                end
        ...
        function updateSettingDisplay(obj,varargin)
            ...
            if ~obj.advancedMode && ~obj.settingsAreSimple
                obj.advancedMode = 1;
            end
            ...
            if isempty(obj.hScan2D)
                ...
                if obj.isH
                    d = arrayfun(@(n,s){{sprintf(' CH%d',n) [' ' upper(s.source)] s.mode ~logical(s.disableDivide)...
                        logical(s.laserGate) sprintf(' [%s %s]', num2str(s.laserFilterWindow(1)), num2str(s.laserFilterWindow(2)))}'},1:Nv,vcs);
                    ...
                    d = arrayfun(@(n,s){{sprintf(' CH%d',n) [' ' upper(s.source)] logical(s.threshold) s.thresholdValue logical(s.binarize) logical(s.edgeDetect) logical(s.laserGate)...
                        sprintf(' [%s %s]', num2str(s.laserFilterWindow(1)), num2str(s.laserFilterWindow(2)))}'},1:Nv,vcs);
                    if numel(d) < obj.hScan2D.MAX_NUM_CHANNELS
                        d{end+1} = {'    +' '' '' '' '' '' '' ''}';
                    end
                end
        ...
        function simplifySettings(obj)
            v = obj.hScan2D.virtualChannelSettings;
            wnd = reshape([v.laserFilterWindow],2,[])';
            wnd = [min(wnd(:,1)) max(wnd(:,2))];
            laserGate = any([v.laserGate]);
            
            v = obj.hScan2D.defaultVirtChanSettings();
            for i = 1:numel(v)
                v(i).laserGate = laserGate;
                v(i).laserFilterWindow = wnd;
            end
            
            obj.hScan2D.virtualChannelSettings = v;
        end
        ...
        function readSamples(obj,~,data)
            ...
            if obj.isH
                ...
                if ~isempty(obj.laserSel) && isfield(data.triggers,'SyncTrigger')
                    filteredlaser = logical(data.triggers.SyncTrigger);
                else
        ...
        function wndHit(obj,src,evt)
            ...
            elseif strcmp(evt.EventName, 'WindowMouseMotion')
                if obj.advancedMode
                    obj.hScan2D.virtualChannelSettings(id).laserFilterWindow(inds) = ov + round(obj.hWndoAxes.CurrentPoint(1) - op);
                else
                    obj.samplingWindow(inds) = ov + round(obj.hWndoAxes.CurrentPoint(1) - op);
                end
            else
        ...
        function updateWindowDisp(obj)
            if isempty(obj.triggerNominalPeriodTicks)
                ...
                for i = 1:64
                    ...
                    if obj.advancedMode && (numel(sa) >= i)
        ...
        function setImgSys(obj,varargin)
            ...
            if isa(obj.hSI.hScan2D, 'scanimage.components.scan2d.RggScan')
                ...
                if obj.isH
                    ...
                    obj.laserTriggerPhase = obj.hFpga.syncTrigDelayAdjust;
                    ...
                    obj.hPhysChannelTable.ColumnName = {'' 'Range' 'Invert' 'Subtract Offset' 'Offset Value' 'Filter'};
                    obj.hPhysChannelTable.ColumnWidth = {25 50 40 87 73 56};
                    obj.hPhysChannelTable.ColumnEditable = [false true true true true true];
                    obj.hPhysChannelTable.ColumnFormat = {'char' inputRgFmt 'logical' 'logical' 'numeric' 'char'};
                ...
                if obj.isH
                    obj.hVirtChannelTable.ColumnName = {'' 'Source' 'Mode' 'Avg' 'Laser Filt' 'Window'};
                    obj.hVirtChannelTable.ColumnWidth = {36 50 110 32 54 60};
                    obj.hVirtChannelTable.ColumnEditable = [false true true true true true];
                    obj.hVirtChannelTable.ColumnFormat = {'char' ops {'analog' 'photon counting'} 'logical' 'logical' 'char'};
                else
                    obj.hVirtChannelTable.ColumnName = {'' 'Source' 'Threshold' 'Threshold' 'Binarize' 'Edge Detect' 'Laser Filt' 'Window'};
                    obj.hVirtChannelTable.ColumnWidth = {36 50 60 60 50 72 50 60};
                    obj.hVirtChannelTable.ColumnEditable = [false true true true true true true true];
                    obj.hVirtChannelTable.ColumnFormat = {'char' ops 'logical' 'numeric' 'logical' 'logical' 'logical' 'char'};
                end
                obj.virtualSettingsChanged();
        ...
        function advancedClockSettings(obj,varargin)
            if obj.isH
                ...
                if ~most.idioms.isValidObj(obj.hAdvClkFig)
                    ...
                    figure(obj.hAdvClkFig)
                end
                obj.updatePhsSettingDisplay();
            end
            
            function makeFig()
                obj.hAdvClkFig = figure('Name', 'Advanced Clock Settings', 'CloseRequestFcn', @hideFig, 'Position', most.gui.centeredScreenPos([295 86]),...
                    'numbertitle','off','MenuBar', 'none', 'Resize', 'off');
    ...
    methods (Access = protected)
        function initGui(obj)
            ...
            most.gui.uicontrol('parent',rf,'string','Apply Clock Settings','WidthLimits',120,'callback',@obj.applyClockSettings);
            ...
            most.gui.uicontrol('parent',rf,'style','edit','Bindings',{obj 'triggerNominalPeriodTicksStr' 'string'},'BackgroundColor',.95*ones(1,3),'enable','inactive');
            ...
            obj.upSimple = most.gui.uipanel('parent',lf,'bordertype','none','WidthLimits',200);
            ifl = most.gui.uiflowcontainer('Parent',obj.upSimple,'FlowDirection','TopDown','margin',0.00001);
            u = most.gui.uipanel('parent',ifl,'title','Laser Trigger Filter');
            ifl = most.gui.uiflowcontainer('Parent',u,'FlowDirection','TopDown','margin',0.00001);
            most.gui.uicontrol('parent',obj.upSimple,'style','checkbox','string','Advanced','RelPosition',[120 18 70 20],'Bindings',{obj 'advancedMode' 'value'});
            most.gui.uipanel('parent',ifl,'bordertype','none','HeightLimits',4);
            rf = most.gui.uiflowcontainer('Parent',ifl,'FlowDirection','LeftToRight','HeightLimits',24);
            most.gui.staticText('parent',rf,'HorizontalAlignment','right','string','Enable Filter:','WidthLimits',110);
            most.gui.uicontrol('parent',rf,'style','checkbox','Bindings',{obj 'enableSampleMask' 'value'},'WidthLimits',16);
            rf = most.gui.uiflowcontainer('Parent',ifl,'FlowDirection','LeftToRight','HeightLimits',24);
            most.gui.staticText('parent',rf,'HorizontalAlignment','right','string','Window Start (Tick):','WidthLimits',110);
            most.gui.uicontrol('parent',rf,'style','edit','Bindings',{obj 'wndStart' 'value'},'KeyPressFcn',@obj.keyFunc);
            rf = most.gui.uiflowcontainer('Parent',ifl,'FlowDirection','LeftToRight','HeightLimits',24);
            most.gui.staticText('parent',rf,'HorizontalAlignment','right','string','Window End (Tick):','WidthLimits',110);
            most.gui.uicontrol('parent',rf,'style','edit','Bindings',{obj 'wndEnd' 'value'},'KeyPressFcn',@obj.keyFunc);
            iflb = most.gui.uiflowcontainer('Parent',ifl,'FlowDirection','BottomUp','margin',0.00001);
            rf = most.gui.uiflowcontainer('Parent',iflb,'FlowDirection','RightToLeft','HeightLimits',28);
            most.gui.uicontrol('Parent',rf,'string','Save Filter Settings','callback',@obj.saveFilterSettings,'WidthLimits',150);
            %                                 most.gui.uicontrol('Parent',rf,'string','Revert Settings','callback',@obj.revertSettings);
            
            obj.upAdvanced = most.gui.uipanel('parent',lf,'bordertype','none','WidthLimits',484,'Visible','on');
            ...
            ifl = most.gui.uiflowcontainer('Parent',u,'FlowDirection','TopDown','margin',0.00001);
            most.gui.uicontrol('parent',obj.upAdvanced,'style','checkbox','string','Advanced','RelPosition',[387 18 70 20],'Bindings',{obj 'advancedMode' 'value'});
            tf = most.gui.uiflowcontainer('Parent',ifl,'FlowDirection','TopDown','margin',4);
            obj.hVirtChannelTable = uitable('Parent',tf,'rowname',{},'CellEditCallback',@obj.virtualTableCb);
    ...
    methods
        function set.triggerNominalPeriodTicks(obj,v)
            ...
            if ~isempty(v)
                ...
                obj.triggerNominalPeriodTicksStr = num2str(v);
            else
                obj.triggerNominalPeriodTicksStr = '';
            end
            if isempty(obj.lastWindowUpdatePeriod) || (~isequal(obj.lastWindowUpdatePeriod, obj.triggerNominalPeriodTicks))
                obj.updateWindowDisp();
                obj.lastWindowUpdatePeriod = obj.triggerNominalPeriodTicks;
            end
        end
        ...
        function set.laserTriggerPhase(obj,v)
            ...
            if ~obj.initmode
                obj.hFpga.syncTrigDelayAdjust = v;
            end
        end
        ...
        function set.advancedMode(obj,v)
            if ~v && ~obj.settingsAreSimple
                if strcmp(questdlg('Current settings can only be used in advanced mode. Revert to simple configuration?','Laser Trigger Scope','Yes','Cancel','Cancel'),'Yes')
                    obj.simplifySettings();
                else
                    v = 1;
                end
            end
            obj.advancedMode = v;
            obj.upAdvanced.Visible = most.idioms.ifthenelse(v,'on','off');
            obj.upSimple.Visible = most.idioms.ifthenelse(v,'off','on');
            obj.updateSettingDisplay();
        end
