classdef  SlmCalibrationControls < most.Gui    
    ...
    methods (Access = protected)
        function initGui(obj)
            ...
            if ~isempty(obj.hModel.hSlmScan) && isvalid(obj.hModel.hSlmScan)
                obj.referencePixelValue = double(intmax(obj.hModel.hSlmScan.hSlm.hDevice.pixelDataType));
            end
            ...
            obj.hAx1 = most.idioms.axes('Parent',hPanelAx1,'Box','on');
            
            hPanelAx2 = uipanel('Parent',hAx2Container,'Title','SLM Phase Response','FontWeight','bold');
            obj.hAx2 = most.idioms.axes('Parent',hPanelAx2,'Box','on');
            ...
            hPanelFlCtr = most.gui.uiflowcontainer('Parent',hPanel,'FlowDirection','TopDown');
                ...
                hPanelFlCtr4 = most.gui.uiflowcontainer('Parent',hPanelFlCtr,'FlowDirection','LeftToRight');
                    obj.pmhChannel = obj.addUiControl(...
                        'Parent',hPanelFlCtr4,...
                        'Style','popupmenu',...
                        'Tag','pmhChannel',...
    ...
    %% User Methods
    methods
        function changeChannel(obj,varargin)
            channel = obj.pmhChannel.pmValue;
            
            try
                if most.idioms.isValidObj(obj.hModel.hSlmScan)
                    obj.hModel.hSlmScan.hLutCalibrationAI = channel;
                end
            catch ME
                obj.channelChanged();
                most.ErroHandler.rethrow(ME);
            end
        ...
        function channelChanged(obj)
            ...
            obj.pmhChannel.String = [{''}, hResourceStore.filterByClass('dabs.resources.ios.AI')];
            
            if most.idioms.isValidObj(obj.hModel.hSlmScan)
                obj.pmhChannel.pmValue = obj.hModel.hSlmScan.hLutCalibrationAI;
            else
                obj.pmhChannel.pmValue = '';
            end
        end
        
        function measureSlmResponse(obj)
            if ~isempty(obj.hModel.hBeams)
                h = msgbox('User Action required: Set Power fraction of beam modulators to illuminate the SLM','Beam Modulators','warn');
                waitfor(h);
            end
        ...
        function plotLut(obj)
            ...
            hAx_.XLim = [0 2*pi];
            hAx_.YLim = [0 2^obj.hModel.hSlmScan.hSlm.hDevice.pixelBitDepth-1];
            ticks = 0:(.25*pi):2*pi;
            hAx_.XTick = ticks;
