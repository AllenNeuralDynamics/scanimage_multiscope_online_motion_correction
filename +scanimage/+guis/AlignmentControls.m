classdef AlignmentControls < most.Gui & most.HasClassDataFile
    ...
    methods (Access = protected)
        function initGui(obj)
           set(obj.hFig,'Name','ALIGNMENT CONTROLS','Resize','off');
            ...
            obj.hListeners = [...
                most.ErrorHandler.addCatchingListener(obj.hModel.hDisplay,'renderer','PostSet',@obj.rendererModeChanged);...
                most.ErrorHandler.addCatchingListener(obj.hModel.hDisplay,'chan1LUT','PostSet',@obj.videoLutChanged);...
                most.ErrorHandler.addCatchingListener(obj.hModel.hDisplay,'chan2LUT','PostSet',@obj.videoLutChanged);...
                most.ErrorHandler.addCatchingListener(obj.hModel.hDisplay,'chan3LUT','PostSet',@obj.videoLutChanged);...
                most.ErrorHandler.addCatchingListener(obj.hModel.hDisplay,'chan4LUT','PostSet',@obj.videoLutChanged);...
                most.ErrorHandler.addCatchingListener(obj.hModel.hUserFunctions,'frameAcquired',@(varargin)obj.frameAcquired());...
                most.ErrorHandler.addCatchingListener(obj.hModel,'imagingSystem','PostSet',@(varargin)obj.updateChannelOptions());...
                most.ErrorHandler.addCatchingListener(obj.pmVideoSource,'ButtonDown',@(varargin)obj.updateChannelOptions)];
            
            obj.hStimRoiGroupListener = most.util.DelayedEventListener(0.3,obj.hModel.hPhotostim,'stimRoiGroups','PostSet',@(varargin)obj.updatePhotostimOptions());
            
            %listeners to cameras
            obj.hListeners(end+1) = most.ErrorHandler.addCatchingListener(obj.hModel.hCameraManager,...
    ...
    methods
        ...
        function updateChannelOptions(obj)
            ...
            hasPhotostimSource = obj.hModel.hPhotostim.numInstances > 0;
            if hasPhotostimSource
...
%             end
        ...
        function updatePhotostimOptions(obj,varargin)
            obj.stimRoiGroupNameListeners.delete();
            obj.stimRoiGroupNameListeners = event.listener.empty(0,1);
            ...
            if isempty(obj.hModel.hPhotostim.stimRoiGroups)
                ...
                obj.stimRoiGroupNameListeners = most.ErrorHandler.addCatchingListener(obj.hModel.hPhotostim.stimRoiGroups, 'name', 'PostSet', @obj.updatePhotostimOptions);
    ...
    methods
        ...
        function set.videoImToRefImTransform(obj,val)
            ...
            [offsetX,offsetY,scaleX,scaleY,rotation,shear] =...
                scanimage.mroi.util.paramsFromTransform(obj.videoImToRefImTransform);
        ...
        function set.alignmentSource(obj,val)
            ...
            camerasExist = ~isempty(obj.hModel.hCameraManager)...
                && ~isempty(obj.hModel.hCameraManager.hCameraWrappers);
            if camerasExist
                hCameras = obj.hModel.hCameraManager.hCameras;
                cameraNames = cell(size(hCameras));
                for iCamera=1:length(hCameras)
                    cameraNames{iCamera} = hCameras{iCamera}.cameraName;
                end
            else
                cameraNames = {};
            end
            matchesCameraName = strcmp(val, cameraNames);
            
            if strcmp(val,'Photostimulation')                
            ...
            elseif any(matchesCameraName)
                % Do camera specific stuff
                hCameraWrappers = obj.hModel.hCameraManager.hCameraWrappers;
                obj.videoStreamSourceChan = hCameraWrappers(matchesCameraName);
                ...
                ch = str2double(regexpi(val,'(?<=^Channel\s)[0-9]*$','match','once'));
                if ~isempty(ch) && ~isnan(ch)
                    ...
                    stimPathVisible = 'off';
                elseif strcmpi(val,'SLM Test Pattern')
                    obj.testPattern = obj.hModel.hSlmScan.testPattern;

                    obj.pbPause.Visible = 'off';
                    stimPathVisible = 'off';
                elseif strcmpi(val,'SLM Points')
                    Pts = obj.hModel.hSlmScan.hSlm.hPtLastWritten;
                    Pts = Pts.transform(obj.hModel.hSlmScan.hCSCoordinateSystem.hParent);
                    obj.slmPoints = Pts.points;
                ...
                obj.updateFigureImage();
                set(obj.hControlPoints, 'Visible','on');
                set(obj.hStimPth, 'Visible',stimPathVisible);
                
                obj.hMainContextMenuAEs.Enable = 'on';
                obj.hMainContextMenuAFx.Enable = 'on';
                obj.hMainContextMenuAFr.Enable = 'on';
            end
            obj.alignmentSource = val;
        end
    ...
    methods
        ...
        function generateRefTransformFromGeometryProps(obj,param,val)
            ...
            T = scanimage.mroi.util.paramsToTransform(...
                gP.offsetX,gP.offsetY,gP.scaleX,gP.scaleY,gP.rotation,gP.shear);
        ...
        function saveScannerTransform(obj)
            if ~isempty(obj.testPattern)
                ...
                assert(isfield(obj.videoData, 'scanner') && ~isempty(obj.videoData.scanner) && scannerExists(obj.videoData.scanner), 'Not enough information to compute transforms. Video stream was collected from unknown scanner.');
            ...
            elseif ~isempty(obj.slmPoints)
                hTargetScanner = obj.hModel.hSlmScan;
            ...
            if ismethod(hTargetScanner,'saveCalibration')
                hTargetScanner.saveCalibration();
            end
            
			for idx = 1:numel(obj.controlPointPositions)
                obj.controlPointPositions{idx} = scanimage.mroi.util.xformPoints(obj.controlPointPositions{idx},obj.videoImToRefImTransform);
            end
            
            obj.videoImToRefImTransform = eye(3);
			
            function tf = scannerExists(scannerName)
                scannerNames = cellfun(@(hR)hR.name,obj.hModel.hScanners,'UniformOutput',false);
                tf = any(strcmp(scannerName,scannerNames));
            end
        end
    ...
    methods (Hidden)
        ...
        function updateFigureImage(obj, force)
            ...
            if obj.showWindow
                if obj.vidDirty
                    if isempty(obj.videoStreamSourceChan)
                        if ~isempty(obj.slmPoints)
                            pts = scanimage.mroi.util.xformPoints(obj.slmPoints(:,1:2),obj.videoImToRefImTransform);
                            for i = 1:size(pts,1)
                                hL = obj.hStimPth(i);
                                hL.XData = pts(i,1);
                                hL.YData = pts(i,2);
                            end
                        elseif ~isempty(obj.hStimPth)
                        ...
                        elseif ~isempty(obj.testPattern)
                            ...
                            surfMeshXX = linspace(0.5, xres + 0.5, 10);
                            surfMeshYY = linspace(0.5, yres + 0.5, 10);
                            [surfMeshXX,surfMeshYY] = meshgrid(surfMeshXX, surfMeshYY);
                            % surf is rotated to avoid transpose later
                    ...
                    if ~isempty(obj.videoData)
                        ...
                        for i = 1:Nroi
                            if numel(obj.hVidIm) < i
                                obj.hVidIm(i) = surface(ones(2),ones(2),ones(2),...
                                    'Parent', obj.hAlignmentAx,...
                                    'Hittest', 'off',...
                                ...
                                obj.hVidImOutline(i) = line(NaN,NaN,NaN,...
                                    'Parent', obj.hAlignmentAx,...
                                    'Hittest', 'off',...
                                    'Color','b');
                            end
                            ...
                            % surf is rotated to avoid transpose later
                            surfMeshXX = obj.videoData.roiDat(i).surfMeshXX;
                            surfMeshYY = obj.videoData.roiDat(i).surfMeshYY;
                            [surfMeshXX,surfMeshYY] =...
                                scanimage.mroi.util.xformMesh(...
                                surfMeshXX, surfMeshYY, obj.videoImToRefImTransform);
                ...
                if obj.refDirty && ~isempty(obj.referenceData)
                    ...
                    for i = 1:Nroi
                        if numel(obj.hRefIm) < i
                            obj.hRefIm(i) = surface(ones(2),ones(2),zeros(2),...
                                'Parent',obj.hAlignmentAx,...
                                'Hittest','off',...
                                'FaceColor','texturemap',...
                                'EdgeColor','none',...
                                'FaceLighting','none',...
                                'FaceAlpha',1);
                            obj.hRefImOutline(i) = line(NaN,NaN,NaN,...
                                'Parent',obj.hAlignmentAx,...
                                'Hittest','off',...
                                'Color',[0 0 .5]);
                        end
        ...
        function resetAlignmentFig(obj,show)
            ...
            most.idioms.safeDeleteObj(obj.hAlignmentAx);
            obj.hAlignmentAx = most.idioms.axes('Parent',obj.hAlignmentFig,'Position',[0 0 1 1],'UIContextMenu',obj.hMainContextMenu, ...
        ...
        function loadPhotostimPath(obj,varargin)
            ...
            if Nr > 0
                ...
                for idx = 1:numel(ss.beams)
                    ss.beams(idx).sampleRateHz = sampleRate;
                end
                
                for idx = 1:numel(ss.fastz)
                    ss.fastz(idx).sampleRateHz = sampleRate;
                end
        ...
        function clearSlmPts(obj)
            obj.clearPhotostimPath();
        end
        
        function loadSlmPts(obj)
            obj.clearSlmPts();
            numPts = size(obj.slmPoints,1);
            obj.pathPointPositions = {};
            obj.pathPointFixed = false(0,1);
            
            if numPts > 1
                for idx = 1:numPts
                    obj.hStimPth(idx) = line(0,0,1,'parent',obj.hAlignmentAx,'linestyle','none','linewidth',2,'color',[1 0 1],'MarkerSize',30,'Marker','.');
                    obj.hStimPth(idx).ButtonDownFcn = @(varargin)obj.cpFunc(obj.hStimPth(idx),true);
                    
                    obj.hPathMenus(idx) = uicontextmenu('Parent',obj.hAlignmentFig);
                    uimenu('Parent',obj.hPathMenus(idx),'Label','Fix SLM Point','Callback',@(src,evt)obj.toggleControlPointFixState(obj.hStimPth(idx)));
                    obj.hStimPth(idx).UIContextMenu = obj.hPathMenus(idx);
                    
                    obj.pathPointPositions{idx} = obj.slmPoints(idx,1:2);
                    obj.pathPointFixed(idx) = false;
                    obj.vidDirty = true;
                end
