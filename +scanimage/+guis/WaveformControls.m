classdef WaveformControls < most.Gui
        ...
        function updateButtonStatus(obj)
            ...
            if ~isempty(obj.hModel)
                ...
                elseif ~ss.sensorCalibrated(prefix)
                    ...
                    set([obj.pbTest obj.pbOptimize], 'Enable', 'off');
                else
                    obj.pbCalibrate.hCtl.BackgroundColor = .94*ones(1,3);
                    set([obj.pbTest obj.pbOptimize obj.pbCalibrate], 'Enable', 'on');
                end
            end
            
            function na()
                obj.pbCalibrate.hCtl.BackgroundColor = .94*ones(1,3);
                set([obj.pbTest obj.pbOptimize obj.pbCalibrate], 'Enable', 'off');
            end
        end
    ...
    methods
        function set.optMode(obj,v)
            ...
            set([obj.pbOptimize obj.pbReset], 'Visible', obj.tfMap(~v));
        ...
        function set.testMode(obj,v)
            ...
            set([obj.pbRefresh obj.pbOptimize obj.pbReset], 'Enable', obj.tfMap(~v));
