    ...
    methods
        function obj = RoiDisplay(hSI,parent,chan)
            ...
            obj.hSurfContextMenu = handle(uicontextmenu('Parent',obj.hFig,'Callback',@obj.contextMenuOpen));
                ...
                hideInMergeWindow(end+1) = uimenu('Parent',obj.hSurfContextMenu,'Label','Add roi as motion correction ref','Callback',@(src,evt)obj.surfContextMenuCallback(@obj.addMotionReferenceThisRoi));
                hideInMergeWindow(end+1) = uimenu('Parent',obj.hSurfContextMenu,'Label','Set roi slice as motion correction ref','Callback',@(src,evt)obj.surfContextMenuCallback(@obj.setMotionReferenceThisRoiSlice));
                hideInMergeWindow(end+1) = uimenu('Parent',obj.hSurfContextMenu,'Label','Enable motion correction','Tag','uiMenuMotionCorrectionEnabled','Callback',@(src,evt)obj.toggleEnableMotionCorrection);
...
%             end
    ...
    methods
        ...
        function dragStage(obj,hSurf)            
            ...
            function tf = isContinuousMoveAllowed()
                % continuos move is allowed if all axes are Zaber or Simulated axes
                motorAxes = obj.hSI.hMotors.motorAxes;
                hMotors = {motorAxes.hMotor};
                motorIsZaber     = cellfun(@(hMotor)isa(hMotor,'dabs.zaber.ZaberMultiDevice'),hMotors);
                motorIsSimulated = cellfun(@(hMotor)isa(hMotor,'dabs.simulated.Motor'),hMotors);
