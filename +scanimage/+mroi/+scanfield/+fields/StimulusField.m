classdef StimulusField < scanimage.mroi.scanfield.ScanField
    ...
    properties (SetObservable)
        ...
        powerFractions = 0.1;% array of power fraction of each beam
        slmPattern = zeros(0,4);  % [Nx4] array of [x,y,z,weight] point or [NxM] bitmap to generate with SLM during this stimulus function
    ...
    properties (SetObservable, Dependent)
        powers;              % legacy, array of power of each beam in percent
    end
    ...
    methods
        ...
        function set.duration(obj,val)
            validateattributes(val,{'numeric'},{'scalar','positive','finite','real'});
            obj.duration = val;
            obj.fireChangedEvent();
        end
        ...
        function set.powerFractions(obj,val)
            if isempty(val)
                ...
                validateattributes(val,{'numeric'},{'row'});
                validateattributes(val(~isnan(val)),{'numeric'},{'>=',0,'<=',1,'real'});
            end
