function [phi,efficiency] = GSW_GPU(scanner,pts,w)
    startTime = tic();
    ...
    z_scaled_gpu = gpuArray(single( pts(:,3)*pi   / (wavelength_um * focalLength_um^2 * n) ));
    ...
    for k = 1:numIterations % iterative optimization
        ...
        phaseMask = calculatePhaseMask(pixPosXX_gpu,pixPosYY_gpu,x_scaled_gpu,y_scaled_gpu,z_scaled_gpu,F);
        V_c = calculateV_c(phaseMask,pixPosXX_gpu,pixPosYY_gpu,beamProfile_gpu,x_scaled_gpu,y_scaled_gpu,z_scaled_gpu);
    end
...
function phaseMask = calculatePhaseMask(pixPosXX,pixPosYY,x_scaled,y_scaled,z_scaled,F)
    M = uint32(numel(x_scaled));
    F = cast(F,'like',x_scaled);
    F = complex(F); % ensure F is complex (otherwise performance goes down)
    
    phaseMask = arrayfun(@calculatePixelPhase,pixPosXX,pixPosYY,M);

    %%% GPU stencil function
    function pixPhase = calculatePixelPhase(pixPosX,pixPosY,M)
        pixD = pixPosX^2 + pixPosY^2;
        pixPhase_complex = complex(single(0));
        pt_idx = uint32(1);
        
        while pt_idx <= M
            pt_pix_Phase = x_scaled(pt_idx)*pixPosX + y_scaled(pt_idx)*pixPosY + z_scaled(pt_idx)*pixD;
...
function V_c = calculateV_c(phaseMask,pixPosXX,pixPosYY,beamProfile,x_scaled,y_scaled,z_scaled)
    M = numel(x_scaled);    
    V_c = complex(zeros(M,1,'like',x_scaled));
    
    for m = 1:M
        V_c_2D = arrayfun(@calculateV_c_2D,pixPosXX,pixPosYY,phaseMask,beamProfile,x_scaled(m),y_scaled(m),z_scaled(m));
    ...
    %%% GPU stencil function
    function V_c_pix = calculateV_c_2D(pixPosX,pixPosY,phaseMask,beamPower,x_scaled,y_scaled,z_scaled)
        pixD = pixPosX^2 + pixPosY^2;
        pixPhase = x_scaled*pixPosX + y_scaled*pixPosY + z_scaled*pixD;
