classdef SLM < handle
    ...
    methods (Hidden)
        ...
        function xy_um = angleDegCSReferenceToObjectiveDistanceUm(obj,xy_deg)
            ...
            xy_um = xy_deg; % fill in z coordinates if existant
            xy_rad = pi * xy_deg(:,1:2)/180;
            xy_um(:,1:2) = obj.focalLength_um*tan(xy_rad);
        end
        
        function xy_deg = distanceObjectiveUmToAngleDegCSReference(obj,xy_um)
            % this is a bit of a weird conversion because of the way that
            % the reference space in ScanImage is defined:
            % the reference space is defined in angles (lateral) and um
            % (axial). This means we only need to convert XY um to XY angle
            % here and don't need to touch the Z coordinate
            xy_deg = xy_um; % fill in z coordinates if existant
            xy_rad = atan(xy_um(:,1:2)/obj.focalLength_um);
            xy_deg(:,1:2) = 180 * xy_rad/pi;
        end
    end
    ...
    methods (Hidden)        
        ...
        function geometryBuffer_ = updateGeometryBuffer(obj)
            ...
            if isempty(obj.hDevice.pixelResolutionXY) || isempty(obj.hDevice.pixelResolutionXY)...
    ...
    methods
        ...
        function val = get.angularRangeXY(obj)            
            val = atan( (obj.wavelength_um/2) ./ obj.hDevice.pixelPitchXY_um );
            val = 180 * val/pi;
            val = 2 * val;
        end
        
        function val_um = get.scanDistanceRangeXYObjective(obj)
            val_deg = obj.angularRangeXY / 2;
            val_um = obj.angleDegCSReferenceToObjectiveDistanceUm(val_deg) * 2;
        end
