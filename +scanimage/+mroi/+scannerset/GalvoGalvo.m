classdef GalvoGalvo < scanimage.mroi.scannerset.ScannerSet
    ...
    methods
        function obj = GalvoGalvo(name,galvox,galvoy,beams,fastZs,fillFractionSpatial,pixelTime,bidirectional,stepY,settleTimeFraction)
            %% Describes a galvo-galvo scanner set.
            obj = obj@scanimage.mroi.scannerset.ScannerSet(name,beams,fastZs);
        ...
        function path_FOV = interpolateTransits(obj,path_FOV,tuneZ,zWaveformType)
            ...
            for idx = 1:numel(obj.fastz)
                path_FOV.Z(:,idx) = obj.fastz(idx).interpolateTransits(obj,path_FOV.Z(:,1),tuneZ,zWaveformType);
        ...
        function path_FOV = padFrameAO(obj, path_FOV, frameTime, flybackTime, zWaveformType)
            ...
            for idx = 1:numel(obj.fastz)
                path_FOV.Z(:,idx) = obj.fastz(idx).padFrameAO(obj, path_FOV.Z(:,idx), frameTime + flybackTime, zWaveformType);
            end
        end
        ...
        function path_FOV = zFlybackFrame(obj, frameTime)
            ...
            if obj.hasBeams
                bSamples = obj.nsamples(obj.beams(1),frameTime);
                
                path_FOV.B = NaN(bSamples,numel(obj.beams));
                blankMask = logical( [obj.beams.flybackBlanking] );
                path_FOV.B(:,blankMask) = 0;
...
%                 end
                ...
                path_FOV.Z(:,idx) = obj.fastz(idx).scanPathFOV(obj,actz,actzRelative,dzdt,seconds,path_FOV.G);
            end
            ...
            function path_FOV = generateBeamsPathImaging(path_FOV)
                ...
                for beamIdx = 1:numel(obj.beams)
                    ...
                    [powerFractions, pzAdjusts, Lzs, interlaceDecimations, interlaceOffsets] = obj.getRoiBeamProps(...
                        roi, 'powerFraction', 'pzAdjust', 'Lz', 'interlaceDecimation', 'interlaceOffset');
                    
                    powerFraction = powerFractions(beamIdx);
                    pzAdjust = pzAdjusts(beamIdx);
                    interlaceDecimation = interlaceDecimations(beamIdx);
                    interlaceOffset = interlaceOffsets(beamIdx);
                    ...
                    path_FOV.B(:,beamIdx) = powerFracs;
                    
                    if obj.hasPowerBox
                        path_FOV.Bpb(:,beamIdx) = powerFracsPb;
                    end
                end
            end
        end
        ...
        function ao_volts=degrees2volts(obj,path_FOV,iscanner)
            ao_volts = path_FOV * obj.scanners{iscanner}.hDevice.voltsPerDistance;
        end
        
        function path_FOV = volts2degrees(obj,ao_volts,iscanner)
            path_FOV = ao_volts / obj.scanners{iscanner}.hDevice.voltsPerDistance;
        end
    end
