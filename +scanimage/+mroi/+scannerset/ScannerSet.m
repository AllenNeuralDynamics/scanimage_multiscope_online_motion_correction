classdef ScannerSet < handle
    ...
    methods
        ...
        function varargout = getRoiBeamProps(obj, hRoi, varargin)
            ...
            for i = 1:nargout
                propNameBeam = varargin{i};
                if isprop(hRoi,propNameBeam)
                    propNameRoi = propNameBeam;
                else
                    propNameRoi = [propNameBeam 's']; % use plural form
                end
                
                propValRoi = hRoi.(propNameRoi);
                propValBeams = [obj.beams.(propNameBeam)];

                if isempty(propValRoi)
                    propVal = propValBeams;
                else
                    propVal = propValRoi;
                    propVal(isnan(propVal)) = propValBeams(isnan(propVal));
                end
