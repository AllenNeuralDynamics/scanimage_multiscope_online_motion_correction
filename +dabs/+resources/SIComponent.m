classdef SIComponent < dabs.resources.Resource
    properties (Hidden, Transient, SetAccess = private)
        hSI = dabs.resources.SIComponent.empty();
    end
    
    methods
        function obj = SIComponent(name)
    ...
    methods (Hidden)
        function validateConfiguration(obj)
            % checks configuration and sets errorMsg
            % overload if needed
        end
    end
    ...
    methods 
        function val = get.hSI(obj)
            if isempty(obj.hSI)
                obj.hSI = obj.searchForScanImage();
            end
    ...
    methods (Access = private)
        function hSI = searchForScanImage(obj)
