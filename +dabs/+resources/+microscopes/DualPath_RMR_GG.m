classdef DualPath_RMR_GG < dabs.resources.MicroscopeSystem    
    properties (Constant)
        manufacturer = 'Vidrio Technologies';
        detailedName = '<html><center><b> Dual Scan Path Vidrio RMR+Linear System</b> <br /> Shutter, pockels cell, and ETL on each path </center></html>';
    end
    
    methods (Static)
        function names = getDescriptiveNames()
            names = {'Complete Microscopes\Dual path Vidrio RMR+GG; shutter, pockels cell, and ETL on each path'};
        end
    end
    
    %% LIFECYCLE
    methods
        function obj = DualPath_RMR_GG(varargin)
            % No-op
        end
    end
    ...
    methods
        function reinit(obj)
            ...
            hR = makeResonantScanner('RMR Res Scanner');
            hGx = makeGalvo('RMR X Galvo',1,1,1);
            hGy = makeGalvo('RMR Y Galvo',1,2,2);
            hP = makePockels('Imaging Beam',3,3);
            hZ = makePiezo('Imaging ETL',4,4);
            ...
            hZ = makePiezo('Stim ETL',9,9);
            hSScan = makeStimScanner(hGx, hGy, hS, hP, hZ);
            
            if ~isempty(obj.hPhotostim)
            ...
            function hResonantScanner = makeResonantScanner(resName)
                try
                    if obj.vdaqPresent
                        ao = obj.hVdaq.hAOs(5);
                        di = obj.hVdaq.hDIs(1);
