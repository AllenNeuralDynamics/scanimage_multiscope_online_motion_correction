classdef RMR_Basic < dabs.resources.MicroscopeSystem    
    properties (Constant)
        manufacturer = 'Vidrio Technologies';
        detailedName = '<html><center><b> Single Scan Path Vidrio RMR System</b> <br /> Shutter, pockels cell, and piezo </center></html>';
    end
    
    methods (Static)
        function names = getDescriptiveNames()
            names = {'Complete Microscopes\Vidrio RMR, shutter, pockels cell, and piezo'};
        end
    end
    
    %% LIFECYCLE
    methods
        function obj = RMR_Basic(varargin)
            % No-op
        end
    end
    ...
    methods
        function reinit(obj)
            ...
            function hResonantScanner = makeResonantScanner()
                try
                    resonantScannerName = 'RMR Res Scanner';
                    hResonantScanner = [];
            ...
            function hGalvoX = makeGalvoX()
                try
                    galvoXName = 'RMR X Galvo';
                    hGalvoX = [];
                    if isempty(obj.hResourceStore.filterByName(galvoXName))
                        hGalvoX = dabs.generic.GalvoPureAnalog(galvoXName);
                        if hGalvoX.mdfHeadingCreated
                            % set defaults
                            hGalvoX.voltsPerDistance = 1;
                            hGalvoX.travelRange = [-10 10];
                            hGalvoX.parkPosition = -9;
                            hGalvoX.slewRateLimit_V_per_s = Inf;
                            
                            if obj.vdaqPresent
                                hGalvoX.hAOControl = obj.hVdaq.hAOs(1);
                                hGalvoX.hAIFeedback = obj.hVdaq.hAIs(1);
                            end
