classdef DualPath_BScopeRedbox < dabs.resources.MicroscopeSystem    
    properties (Constant)
        manufacturer = 'Thorlabs';
        detailedName = '<html><center><b>Dual Scan Path Thorlabs Bergamo (Redbox)</b> <br />Shutter and pockels cell on each path, piezo, MCM3000, PMT2100''s, BCM</center></html>';
    end
    
    methods (Static)
        function names = getDescriptiveNames()
            names = {'Complete Microscopes\Dual path Thorlabs Bergamo (Redbox); shutter and pockels cell on each path; piezo, MCM3000, and PMTs'};
        end
    end
    
    %% LIFECYCLE
    methods
        function obj = DualPath_BScopeRedbox(varargin)
            %No-op
        end
    end
    ...
    methods
        function reinit(obj)
            ...
            hR = makeResonantScanner('Imaging Res Scanner',1);
            ...
            hGx = makeGalvo('Stim X Galvo',1.5,5,5);
            hGy = makeGalvo('Stim Y Galvo',1.5,6,6);
            hP = makePockels('Stim Beam',7,7);
            ...
            function hResonantScanner = makeResonantScanner(resName,aoInd)
                try
                    ...
                    hResonantScanner = createResource(resName,'dabs.generic.ResonantScannerAnalog','settleTime_s',0.5,...
                        'nominalFrequency_Hz',7910,'angularRange_deg',24,'voltsPerOpticalDegrees',5/24,'hAOZoom',ao,'hDISync',di,'hDOEnable',do);
                catch ME
                    most.ErrorHandler.logAndReportError(ME);
                end
            end
