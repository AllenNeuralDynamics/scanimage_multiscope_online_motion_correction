classdef vDAQR0 < dabs.resources.daqs.vDAQ
    ...
    methods
        function queryDigitalPorts(obj)
            ...
            for idx = 1:numel(obj.hDIs)
                obj.hDIs(idx).lastKnownValue = port1(idx);
            end
            
            for idx = 1:numel(obj.hDOs)
                obj.hDOs(idx).lastKnownValue = port2(idx);
            end
        end
    end
    ...
    methods
        function showBreakout(obj)
            if most.idioms.isValidObj(obj.hBreakout)
                ...
                obj.hBreakout = dabs.resources.configuration.private.vDAQR0Breakout(obj);
            end
        end
