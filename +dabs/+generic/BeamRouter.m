classdef BeamRouter < dabs.resources.Device & most.HasMachineDataFile & dabs.resources.configuration.HasConfigPage & dabs.resources.widget.HasWidget
    ...
    methods
        function useExampleFunctions(obj)            
            obj.functionHandle = @dabs.generic.beamrouter.serialBeams;
            obj.functionHandleCalibration = @dabs.generic.beamrouter.serialBeamsCalibration;
        ...
        function setPowerFractions(obj,fractions)
            ...
            assert(isequal(size(routedFractions),size(fractions)) ...
                ,'Error in function %s: input waveform size and output waveform size are not the same',func2str(obj.functionHandle));
            ...
            assert(all(routedFractions(:)>=0) && all(routedFractions(:)<=1) ...
                ,'Error in function %s: output waveform contains must be in rante [0,1]',func2str(obj.functionHandle));
    ...
    methods
        function waveformOut = route(obj,hBeamsIn,waveformIn)
            ...
            if isempty(obj.functionHandle) || isempty(obj.hBeams)
                return
            end
            ...
            waveformIn = waveformIn(:,Locb);
            
            try
                sz = size(waveformIn);
                waveformIn = obj.functionHandle(waveformIn,obj.hBeams,obj);
                assert(isequal(sz,size(waveformIn)),'Size of waveform must not change.');
            catch ME
                most.ErrorHandler.logAndReportError(ME,'%s: error in function ''%s'': %s',func2str(obj.functionHandle),ME.message);
            end
            
            waveformOut(:,Locb) = waveformIn;
        end
...
s = [...
    ...
    most.HasMachineDataFile.makeEntry('functionHandle','@(waveform,hBeams,varargin)waveform','function')...
    most.HasMachineDataFile.makeEntry('functionHandleCalibration','@(hBeams,varargin)false','function')...
    ];
end
